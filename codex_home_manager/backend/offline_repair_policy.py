...
def related_codex_processes() -> list[dict[str, Any]]:
    ...
    for process in psutil.process_iter(["pid", "name"]):
        ...
        is_chrome_native_messaging_host = (
            "chrome-extension:\\" in normalized
            and (
                "chrome.nativemessaging.in." in normalized
                or "chrome.nativemessaging.out." in normalized
                or "--parent-window=" in normalized
            )
        )
        related = (
            name in {
            ...
            or (
                name == "extension-host.exe"
                and "\\.codex\\plugins\\" in normalized
                and not is_chrome_native_messaging_host
            )
        )
        if related:
            found.append(
