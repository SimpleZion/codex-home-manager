...
packaged_product_version = "1.0.11"
...
def load_product_version() -> str:
    ...
    try:
        ...
        version = os.environ.get("CODEX_HOME_MANAGER_VERSION") or packaged_product_version
    if not isinstance(version, str) or not version.strip():
        raise RuntimeError("Codex Home Manager version is unavailable from package.json")
...
class PortableBackupExportRequest(PreviewBoundRequest):
    passphrase: str = Field(min_length=12, max_length=1024)


class PortableRestorePreviewRequest(BaseModel):
    sourceCodexHome: str = Field(min_length=1)
    passphrase: str = Field(min_length=12, max_length=1024)


class PortableRestoreRequest(PreviewBoundRequest):
    sourceCodexHome: str = Field(min_length=1)
    passphrase: str = Field(min_length=12, max_length=1024)
    acknowledgeCodexRunningRisk: bool = False
...
class SnapshotSummaryResponse(BaseModel):
    ...
    threadHighlights: list[dict[str, Any]]
    catalogVersion: str
    generatedAtMs: int
...
class BackupManifestResponse(BaseModel):
    ...
    foreignPortable: bool | None = None
    portableManifestPath: str | None = None
...
class ThreadTimelineResponse(BaseModel):
    ...
    rolloutPath: str
    fileSize: int
    beforeByte: int | None = None
    nextBeforeByte: int | None = None
    limit: int
    kind: str
    search: str
    hasMore: bool
...
def is_safe_loopback_read(request: Request) -> bool:
    if request.method not in {"GET", "HEAD"}:
        return False
    request_origin = f"{request.url.scheme}://{request.url.netloc}".rstrip("/")
    request_hostname = (request.url.hostname or "").casefold()
    if request_hostname not in {"127.0.0.1", "localhost", "::1"}:
        return False
    origin = request.headers.get("origin")
    if origin and origin.rstrip("/") != request_origin:
        return False
...
async def authorize_browser_api_reads(request: Request, call_next):
    if (
        ...
        and not is_safe_loopback_read(request)
    ):
        try:
...
def portable_restore_payload(
    ...
    return {
        ...
        "portableRestore": True,
        "passphraseBinding": hashlib.sha256(
            b"codex-home-manager-portable-preview\x00" + passphrase.encode("utf-8")
        ).hexdigest(),
    }


def portable_export_payload(codex_home: str | None, backup_id: str, passphrase: str) -> dict[str, Any]:
    return {
        "codexHome": codex_home_value(codex_home),
        "backupId": backup_id,
        "portableExport": True,
        "passphraseBinding": hashlib.sha256(
            b"codex-home-manager-portable-export\x00" + passphrase.encode("utf-8")
        ).hexdigest(),
    }
...
def capabilities(lang: str = Query(default="en")) -> dict[str, Any]:
    ...
    return {
        ...
        "capabilities": [
            ...
            item("preview_portable_backup_export", "POST", "/api/backups/{backup_id}/portable/preview", "Preview creation or replacement of a passphrase-protected Ed25519 portable manifest.", ["backup_id", "passphrase"], "read-only", success_fields=["operationPreviewId", "inputHash", "portableManifestPath", "willOverwrite"]),
            item("export_portable_backup", "POST", "/api/backups/{backup_id}/portable", "Create a signed portable manifest and encrypted Ed25519 private-key envelope for an existing schema 2 backup.", ["backup_id", "passphrase", "X-Codex-Manager-Token", "operationPreviewId", "inputHash"], "writes portable-manifest.json without changing schema 2 manifest", success_fields=["backupId", "portableManifestPath", "publicKeyFingerprint", "signedFileCount"]),
            item("preview_portable_backup_restore", "POST", "/api/backups/{backup_id}/portable/restore/preview", "Verify a portable package and explicitly map its signed source Codex Home to the selected current Codex Home.", ["backup_id", "sourceCodexHome", "passphrase"], "read-only", success_fields=["operationPreviewId", "inputHash", "sourceCodexHome", "targetCodexHome", "restoreTargets", "integrityStatus"]),
            item("restore_portable_backup", "POST", "/api/backups/{backup_id}/portable/restore", "Restore a verified portable backup only after explicit source-to-target Codex Home mapping.", ["backup_id", "sourceCodexHome", "passphrase", "X-Codex-Manager-Token", "operationPreviewId", "inputHash"], "creates pre_restore backup by default and keeps the portable source package immutable", success_fields=["restoredBackupId", "notes"]),
...
@app.get("/api/snapshot/summary", response_model=SnapshotSummaryResponse)
def snapshot_summary(
    ...
    try:
        return build_snapshot_summary(codex_home, sidebar_limit, force_refresh=refresh)
    except FileNotFoundError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
    except Exception as error:
        raise HTTPException(status_code=500, detail=str(error)) from error
...
@app.get("/api/threads/page", response_model=ThreadPageResponse)
def thread_page(
    codex_home: str | None = Query(default=None),
    sidebar_limit: int = Query(default=50, ge=1, le=1000),
    thread_kind: Literal["main", "all", "subagent"] = Query(default="main"),
    filter_mode: Literal["all", "visible", "manual_hidden", "repair", "archived"] = Query(default="all"),
    project_path: str = Query(default="all"),
    search: str = Query(default="", max_length=2000),
    sort_mode: Literal["rank", "updated", "size", "tokens", "project", "title", "status"] = Query(default="rank"),
    ...
    try:
        return query_snapshot_thread_page(
            codex_home_text=codex_home,
            sidebar_limit=sidebar_limit,
...
@app.get("/api/threads/{thread_id}/timeline", response_model=ThreadTimelineResponse)
def thread_timeline(
    thread_id: str,
    codex_home: str | None = Query(default=None),
    before_byte: int | None = Query(default=None, ge=0),
    limit: int = Query(default=80, ge=1, le=200),
    kind: str = Query(default="conversation"),
    search: str = Query(default=""),
    content_limit: int = Query(default=120_000, ge=2_000, le=500_000),
) -> dict[str, Any]:
    try:
        return read_thread_timeline(
            codex_home_text=codex_home,
            thread_id=thread_id,
...
@app.get("/api/threads/{thread_id}/timeline/item")
def thread_timeline_item(
    thread_id: str,
    byte_offset: int = Query(ge=0),
    codex_home: str | None = Query(default=None),
    content_limit: int = Query(default=2_000_000, ge=2_000, le=5_000_000),
) -> dict[str, Any]:
    try:
        return read_thread_timeline_item(codex_home, thread_id, byte_offset, content_limit)
    except KeyError as error:
        raise HTTPException(status_code=404, detail=f"thread not found: {thread_id}") from error
    except FileNotFoundError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
...
@app.post("/api/backups/{backup_id}/portable/preview", response_model=PreviewResponse)
def preview_portable_backup_export_endpoint(
    backup_id: str,
    request: PortableBackupExportRequest,
    codex_home: str | None = Query(default=None),
) -> dict[str, Any]:
    try:
        matched_backup = next(
        ...
        return {
            **create_preview_ticket(
                ...
                portable_export_payload(codex_home, backup_id, request.passphrase),
            ),
            "backupId": backup_id,
            "action": matched_backup.get("action"),
            "threadId": matched_backup.get("threadId"),
            "sourceCodexHome": matched_backup.get("codexHome"),
            "sourcePath": matched_backup.get("manifestPath"),
            "portableManifestPath": str(backup_manifest_path(backup_id).parent / "portable-manifest.json"),
            "willOverwrite": (backup_manifest_path(backup_id).parent / "portable-manifest.json").exists(),
            "warnings": [],
        }
    except FileNotFoundError as error:
...
@app.post("/api/backups/{backup_id}/portable")
def export_portable_backup(
    ...
    try:
        ...
        with preview_bound_write(
            ...
            return make_backup_portable(backup_id, codex_home, request.passphrase)
    except HTTPException as error:
        raise error
    except FileNotFoundError as error:
        raise HTTPException(status_code=404, detail=f"backup not found: {backup_id}") from error
...
@app.post("/api/backups/{backup_id}/portable/restore/preview", response_model=PreviewResponse)
def preview_portable_restore_endpoint(
    backup_id: str,
    request: PortableRestorePreviewRequest,
    codex_home: str | None = Query(default=None),
) -> dict[str, Any]:
    try:
        result = preview_portable_backup_restore(
            backup_id,
            codex_home,
            request.sourceCodexHome,
            request.passphrase,
        )
        result.update(
            create_preview_ticket(
                "restore_portable_backup",
                portable_restore_payload(
                    codex_home,
                    backup_id,
                    request.sourceCodexHome,
                    request.passphrase,
        ...
        raise HTTPException(status_code=404, detail=f"portable backup not found: {backup_id}") from error
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    except RuntimeError as error:
        raise runtime_conflict(error) from error
...
@app.post("/api/backups/{backup_id}/portable/restore", response_model=RestoreResponse)
def restore_portable_backup_endpoint(
    backup_id: str,
    ...
    try:
        authorize_write_request(http_request)
        payload = portable_restore_payload(
            codex_home,
            backup_id,
            request.sourceCodexHome,
            request.passphrase,
        ...
        with preview_bound_write(
            ...
            return restore_backup(
                ...
                acknowledge_codex_running_risk=request.acknowledgeCodexRunningRisk,
                create_backup=request.createBackup,
                portable_passphrase=request.passphrase,
                source_codex_home_mapping=request.sourceCodexHome,
            )
    except HTTPException as error:
        raise error
    except FileNotFoundError as error:
...
def mcp_tool_definitions() -> list[dict[str, Any]]:
    ...
    return [
        ...
        *prompt_index_mcp_tool_definitions(
        ...
        mcp_tool("codex_preview_portable_backup_export", "Preview creating or replacing the passphrase-protected signed portable manifest for an existing backup.", mcp_preview_properties({"backupId": {"type": "string"}, "passphrase": {"type": "string", "minLength": 12, "maxLength": 1024}}), ["backupId", "passphrase"]),
        mcp_tool("codex_preview_portable_backup_restore", "Verify a portable backup and preview explicit source-to-current-Codex-Home restore mapping.", mcp_preview_properties({"backupId": {"type": "string"}, "sourceCodexHome": {"type": "string"}, "passphrase": {"type": "string", "minLength": 12, "maxLength": 1024}}), ["backupId", "sourceCodexHome", "passphrase"]),
        ...
        mcp_tool("codex_export_portable_backup", "Create the signed portable manifest and encrypted signing-key envelope for an existing backup.", mcp_write_properties({"backupId": {"type": "string"}, "passphrase": {"type": "string", "minLength": 12, "maxLength": 1024}}), ["backupId", "passphrase", "apiToken", "operationPreviewId", "inputHash"]),
        mcp_tool("codex_restore_portable_backup", "Restore a verified portable backup after explicit source-to-current-Codex-Home mapping.", mcp_write_properties({"backupId": {"type": "string"}, "sourceCodexHome": {"type": "string"}, "passphrase": {"type": "string", "minLength": 12, "maxLength": 1024}}), ["backupId", "sourceCodexHome", "passphrase", "apiToken", "operationPreviewId", "inputHash"]),
...
def mcp_execute_tool(name: str, arguments: dict[str, Any], request: Request) -> dict[str, Any]:
    try:
        ...
        if name == "codex_thread_timeline":
            before_byte_value = arguments.get("beforeByte")
            return mcp_result(read_thread_timeline(
                codex_home_text=codex_home,
                thread_id=mcp_required_str(arguments, "threadId"),
                before_byte=mcp_int(arguments, "beforeByte", 0, 0, None) if before_byte_value is not None else None,
                limit=mcp_int(arguments, "limit", 80, 1, 200),
                kind_filter=mcp_str(arguments, "kind", "conversation") or "conversation",
                search_text=mcp_str(arguments, "search", "") or "",
        ...
        if name == "codex_thread_timeline_item":
            return mcp_result(read_thread_timeline_item(
                codex_home,
                mcp_required_str(arguments, "threadId"),
        ...
        if name == "codex_preview_copy_resource":
            ...
            return mcp_result(result)
        if name == "codex_preview_portable_backup_export":
            backup_id = mcp_required_str(arguments, "backupId")
            request_model = mcp_model(PortableBackupExportRequest, arguments)
            matched_backup = next(
                (backup for backup in list_backups(codex_home) if backup.get("backupId") == backup_id),
                None,
            )
            ...
            manifest_path = backup_manifest_path(backup_id).parent / "portable-manifest.json"
            return mcp_result({
                **create_preview_ticket("export_portable_backup", portable_export_payload(codex_home, backup_id, request_model.passphrase)),
                "backupId": backup_id,
                "action": matched_backup.get("action"),
                "threadId": matched_backup.get("threadId"),
                "sourceCodexHome": matched_backup.get("codexHome"),
                "sourcePath": matched_backup.get("manifestPath"),
                "portableManifestPath": str(manifest_path),
                "willOverwrite": manifest_path.exists(),
                "warnings": [],
            })
        if name == "codex_preview_portable_backup_restore":
            backup_id = mcp_required_str(arguments, "backupId")
            request_model = mcp_model(PortableRestorePreviewRequest, arguments)
            result = preview_portable_backup_restore(
                backup_id,
                codex_home,
                request_model.sourceCodexHome,
                request_model.passphrase,
            )
            result.update(create_preview_ticket(
                "restore_portable_backup",
                portable_restore_payload(codex_home, backup_id, request_model.sourceCodexHome, request_model.passphrase),
            ))
            return mcp_result(result)
        ...
        if name == "codex_export_portable_backup":
            backup_id = mcp_required_str(arguments, "backupId")
            request_model = mcp_model(PortableBackupExportRequest, arguments)
            with preview_bound_write(
                "export_portable_backup",
                portable_export_payload(codex_home, backup_id, request_model.passphrase),
                request_model.operationPreviewId,
                request_model.inputHash,
            ):
                return mcp_result(make_backup_portable(backup_id, codex_home, request_model.passphrase))
        if name == "codex_restore_portable_backup":
            backup_id = mcp_required_str(arguments, "backupId")
            request_model = mcp_model(PortableRestoreRequest, arguments)
            with preview_bound_write(
                "restore_portable_backup",
                portable_restore_payload(codex_home, backup_id, request_model.sourceCodexHome, request_model.passphrase),
                request_model.operationPreviewId,
                ...
                return mcp_result(restore_backup(
                    backup_id,
                    codex_home,
                    acknowledge_codex_running_risk=request_model.acknowledgeCodexRunningRisk,
                    create_backup=request_model.createBackup,
                    portable_passphrase=request_model.passphrase,
                    source_codex_home_mapping=request_model.sourceCodexHome,
                ))
