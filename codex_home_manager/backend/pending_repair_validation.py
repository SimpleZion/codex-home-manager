...
def configured_backup_root() -> Path:
    configured_root = os.environ.get("CODEX_HOME_MANAGER_BACKUP_ROOT")
    if configured_root and configured_root.strip():
        return Path(os.path.expandvars(configured_root.strip())).expanduser().resolve(strict=False)
    local_app_data = os.environ.get("LOCALAPPDATA")
    user_data_root = Path(local_app_data) if local_app_data else Path.home() / "AppData" / "Local"
    return (user_data_root / "CodexHomeManager" / "backups").resolve(strict=False)


def pending_repair_lock_path(backup_root: Path | None = None) -> Path:
    effective_backup_root = (backup_root or configured_backup_root()).resolve(strict=False)
    return effective_backup_root.parent / "codex_full_repair" / "active_repair.lock.json"


default_pending_repair_lock = pending_repair_lock_path()
