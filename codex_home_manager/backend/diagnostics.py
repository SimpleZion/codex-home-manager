        ...
        with connect_database(log_database_path, readonly=True) as connection:
            ...
            rows = connection.execute(
                ...
                WHERE UPPER(level) IN ('ERROR', 'WARN', 'WARNING')
                  ...
                  AND COALESCE(target, '') NOT LIKE 'codex_memories_write%'
...
def scan_node_repl_asset_directories(config_text: str) -> dict[str, Any]:
    ...
    for temporary_directory_key in ("TEMP", "TMP", "TMPDIR"):
        temporary_directory_path = extract_config_path(config_text, temporary_directory_key)
        if temporary_directory_path:
            add_directory(Path(temporary_directory_path))
...
runtime_blocker_patterns: list[tuple[str, re.Pattern[str]]] = [
    ...
    (
        "node_repl_kernel_asset_write_failed",
        re.compile(
            r"failed to write kernel assets"
            r"|(?:内核|kernel).{0,80}(?:资产|assets).{0,80}(?:写入失败|无法写入|write failed)",
            re.IGNORECASE,
        ),
    ),
    (
...
direct_runtime_tool_output_marker = "__codex_home_manager_direct_runtime_tool_output__ "
direct_runtime_tool_output_only_labels = {
    "node_repl_kernel_asset_write_failed",
}
...
def is_node_repl_kernel_asset_write_error(text: str) -> bool:
    ...
    return bool(
        re.search(
            rf"^(?:{failure_pattern})"
            rf"|^Script failed[\s\S]{{0,500}}(?:{failure_pattern})"
            rf"|^(?:Wall time:[^\n]*\n)?Output:\s*(?:\[[\s\S]{{0,500}})?(?:{failure_pattern})",
            stripped_text,
            re.IGNORECASE,
        )
    )
...
def is_targeted_runtime_tool_error(text: str) -> bool:
    ...
    return bool(direct_error or wrapped_tool_output or is_node_repl_kernel_asset_write_error(stripped_text))
    ...
    if record_type == "event_msg":
        ...
        if payload_type == "mcp_tool_call_end" and isinstance(result, dict):
            ...
            if isinstance(error_text, str) and is_targeted_runtime_tool_error(error_text):
                texts.append(f"{direct_runtime_tool_output_marker}{error_text}")
    ...
    if record_type == "response_item":
        ...
        if item_type == "function_call_output":
            ...
            if isinstance(output_text, str) and is_direct_runtime_blocker_output(output_text):
                texts.append(f"{direct_runtime_tool_output_marker}{output_text}")
            return texts
    ...
    for source_path in source_paths:
        ...
        for message_index, normalized_text in enumerate(normalized_texts):
            for label, pattern in runtime_blocker_patterns:
                if (
                    label in direct_runtime_tool_output_only_labels
                    and not normalized_text.startswith(direct_runtime_tool_output_marker)
                ):
                    continue
                for match in pattern.finditer(normalized_text):
                    snippet = re.sub(r"\s+", " ", normalized_text[max(0, match.start() - 120): match.end() + 160]).strip()
                    snippet = snippet.replace(direct_runtime_tool_output_marker, "", 1).strip()
...
def parse_mcp_process_snapshot(processes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for process_index, process in enumerate(processes):
        ...
        node_repl_command_match = re.search(
            r"(?<![a-z0-9_])node_repl(?:\.exe)?(?=[\s\"']|$)",
            command_lower,
        )
        if name_lower in {"node_repl", "node_repl.exe"} or node_repl_command_match:
    ...
    # --disable-sandbox is part of the current Desktop-owned native-pipe runtime.
    # A process snapshot alone cannot distinguish that valid contract from a
    # stale direct-runtime override; the configuration-layer check below can.
    node_repl_risk_keys: set[str] = set()
    ...
    return {
        ...
        "nodeReplRiskProcessCount": len(node_repl_risk_keys),
...
def run_codex_diagnostics(
    ...
    if runtime_blocker_findings:
        ...
        node_repl_kernel_asset_findings = [
            finding
            for finding in runtime_blocker_findings
            if finding.startswith("node_repl_kernel_asset_write_failed")
        ]
        if node_repl_kernel_asset_findings:
            add_issue(
                "plugins.node_repl_kernel_asset_write_failed",
                "warning" if runtime_blocker_status == "warning" else "info",
                "plugins",
                localize(
                    language,
                    "Node REPL kernel asset initialization failed",
                    "Node REPL 内核资产初始化失败",
                ),
                localize(
                    language,
                    "A current or recent node_repl call failed before the JavaScript kernel started because its per-process assets could not be written.",
                    "当前或最近的 node_repl 调用曾在 JavaScript 内核启动前失败，原因是进程级运行资产无法写入。",
                ),
                localize(
                    language,
                    "Verify the configured TEMP, TMP and TMPDIR directories, then run a fresh node_repl or Computer Use probe. Do not delete empty .tmp directories while Codex is running because they may belong to idle node_repl servers.",
                    "核对配置中的 TEMP、TMP 和 TMPDIR 目录，再执行新的 node_repl 或 Computer Use 实连探测。Codex 运行时不要删除空的 .tmp 目录，因为它们可能属于尚未初始化的 node_repl 服务。",
                ),
                node_repl_kernel_asset_findings[:8],
                runtime_blocker_scan["scannedPaths"][:12],
            )
        websocket_findings = [
            finding
    ...
    add_check(
        ...
        [
            ...
            localize(
                language,
                "A live Computer Use connection should import @oai/sky and run a lightweight sky.list_windows() probe; Browser and Chrome use their bundled browser clients.",
                "Computer Use 实连应导入 @oai/sky 并执行轻量 sky.list_windows() 探测；Browser 和 Chrome 使用各自的内置 browser client。",
            ),
            localize(
                language,
    ...
    computer_use_documentation_paths = [
        computer_use_root / "docs" / "guidance.md",
        computer_use_root / "docs" / "api.md",
        computer_use_root / "docs" / "confirmations.md",
    ]
    computer_use_asset_paths = [computer_use_skill_path, *computer_use_documentation_paths]
    ...
    add_check(
        ...
        localize(
            language,
            "Computer Use skill and bundled documentation are present; @oai/sky is loaded from the Desktop Node REPL runtime.",
            "Computer Use 技能和内置文档均存在；@oai/sky 由 Desktop Node REPL 运行时加载。",
        )
        if computer_use_assets_ok
        else localize(language, "Computer Use skill or bundled documentation is missing.", "Computer Use 技能或内置文档缺失。"),
        [
            ...
            *(path_exists_text(path) for path in computer_use_documentation_paths),
        ],
    ...
    if not computer_use_assets_ok:
        add_issue(
            ...
            "warning",
            "plugins",
            localize(language, "Computer Use assets are missing", "Computer Use 资产缺失"),
            localize(language, "Computer Use may be listed as installed, but its skill or bundled documentation is incomplete.", "Computer Use 可能显示为已安装，但技能或内置文档不完整。"),
    ...
    try:
        computer_use_skill_text = computer_use_skill_path.read_text(encoding="utf-8")
    except (OSError, UnicodeError):
        computer_use_skill_text = ""
    computer_use_skill_calls_documentation = "sky.documentation" in computer_use_skill_text
    computer_use_skill_guards_documentation = "typeof sky.documentation" in computer_use_skill_text
    computer_use_skill_has_document_fallback = all(
        marker in computer_use_skill_text
        for marker in ("docs/guidance.md", "docs/api.md", "docs/confirmations.md")
    )
    computer_use_documentation_contract_ok = (
        not computer_use_skill_calls_documentation
        or (
            computer_use_skill_guards_documentation
            and computer_use_skill_has_document_fallback
            and all(path.is_file() for path in computer_use_documentation_paths)
        )
    )
    add_check(
        "plugins.computer_use_documentation_contract",
        "plugins",
        localize(language, "Computer Use documentation contract", "Computer Use 文档契约"),
        "pass" if computer_use_documentation_contract_ok else "warning",
        localize(
            language,
            "The skill safely handles @oai/sky runtimes that do not expose sky.documentation.",
            "技能可安全兼容未暴露 sky.documentation 的 @oai/sky 运行时。",
        )
        if computer_use_documentation_contract_ok
        else localize(
            language,
            "The skill calls sky.documentation without a bundled-document fallback.",
            "技能直接调用 sky.documentation，但没有内置文档回退路径。",
        ),
        [
            f"calls_sky_documentation={computer_use_skill_calls_documentation}",
            f"guards_sky_documentation={computer_use_skill_guards_documentation}",
            f"bundled_document_fallback={computer_use_skill_has_document_fallback}",
        ],
        [str(computer_use_skill_path), *(str(path) for path in computer_use_documentation_paths)],
    )
    if not computer_use_documentation_contract_ok:
        add_issue(
            "plugins.computer_use_documentation_contract_mismatch",
            "warning",
            "plugins",
            localize(language, "Computer Use skill/runtime contract mismatch", "Computer Use 技能与运行时契约不匹配"),
            localize(
                language,
                "The bundled skill can reject a working Computer Use runtime when sky.documentation is absent.",
                "当运行时没有 sky.documentation 时，内置技能可能把实际可用的 Computer Use 误判为不可用。",
            ),
            localize(
                language,
                "Guard the helper call and read docs/guidance.md, docs/api.md and docs/confirmations.md directly when it is absent; verify with sky.list_windows().",
                "为该方法调用增加存在性判断；缺失时直接读取 docs/guidance.md、docs/api.md 和 docs/confirmations.md，并用 sky.list_windows() 实测。",
            ),
            [
                f"calls_sky_documentation={computer_use_skill_calls_documentation}",
                f"guards_sky_documentation={computer_use_skill_guards_documentation}",
    ...
    official_thread_tool_registry = inspect_official_thread_tool_registry(paths)
    official_thread_tool_initial_meta = inspect_official_thread_tool_initial_session_meta(paths)
    non_contiguous_thread_tool_count = int(
        official_thread_tool_registry.get("threadsWithNonContiguousPositions") or 0
    )
    initial_session_meta_issue_count = int(
        official_thread_tool_initial_meta.get("candidateCount") or 0
    )
    actionable_initial_session_meta_issue_count = int(
        official_thread_tool_initial_meta.get("actionableCandidateCount") or 0
    )
    blocked_initial_session_meta_issue_count = int(
        official_thread_tool_initial_meta.get("blockedCandidateCount") or 0
    )
    blocked_initial_session_meta_reason_counts = dict(
        official_thread_tool_initial_meta.get("blockedReasonCounts") or {}
    )
    shared_rollout_alias_count = int(
        blocked_initial_session_meta_reason_counts.get("shared_rollout_alias") or 0
    )
    session_meta_id_mismatch_count = int(
        blocked_initial_session_meta_reason_counts.get("session_meta_id_mismatch") or 0
    )
    initial_session_meta_scan_error = str(official_thread_tool_initial_meta.get("error") or "")
    legacy_thread_messenger_status = (
        "warning"
        if (
            legacy_thread_messenger_blocks
            or managed_legacy_thread_messenger_blocks
            or legacy_thread_messenger_process_count
            or non_contiguous_thread_tool_count
            or initial_session_meta_issue_count
            or initial_session_meta_scan_error
        )
        else "pass"
    )
    legacy_thread_messenger_evidence = [
        ...
        f"non_contiguous_thread_tool_threads={non_contiguous_thread_tool_count}",
        f"initial_session_meta_threads_missing_official_tools={initial_session_meta_issue_count}",
        f"initial_session_meta_actionable_threads={actionable_initial_session_meta_issue_count}",
        f"initial_session_meta_blocked_identity_aliases={blocked_initial_session_meta_issue_count}",
        f"initial_session_meta_shared_rollout_aliases={shared_rollout_alias_count}",
        f"initial_session_meta_id_mismatches={session_meta_id_mismatch_count}",
        f"initial_session_meta_scan_error={initial_session_meta_scan_error or '-'}",
        *[
    ...
    add_check(
        ...
        legacy_thread_messenger_status,
        localize(language, "Official thread tool configuration and initial rollout metadata are aligned.", "官方线程工具配置与线程开头元数据一致。")
        if legacy_thread_messenger_status == "pass"
        else localize(language, "Fallback shadowing, registry position drift, or incomplete initial rollout metadata can hide official codex_app tools.", "检测到 fallback 遮蔽、工具位置漂移或线程开头元数据不完整，可能导致官方 codex_app 工具缺失。"),
        legacy_thread_messenger_evidence,
        [str(paths.config_path), str(managed_config_path), str(paths.database_path)],
    )
    fallback_or_position_issue = bool(
        legacy_thread_messenger_blocks
        or managed_legacy_thread_messenger_blocks
        or legacy_thread_messenger_process_count
        or non_contiguous_thread_tool_count
    )
    if fallback_or_position_issue:
        add_issue(
            ...
            localize(
                language,
                "A legacy fallback can shadow official tools, and non-contiguous thread_dynamic_tools positions can make Desktop stop loading before list_threads/read_thread/send_message_to_thread.",
                "旧 fallback 会遮蔽官方工具；thread_dynamic_tools 位置不连续还会让 Desktop 提前停止加载，导致 list_threads/read_thread/send_message_to_thread 缺失。",
            ),
            localize(
                language,
                "Use Codex Home Manager's official-thread-tools repair while Codex is fully closed, then reopen Desktop and verify visible target-thread delivery plus target-side official tool exposure.",
                "完全关闭 Codex 后使用 Codex Home Manager 的官方线程工具修复，再重新打开 Desktop；成功必须以目标线程可见收到消息、且目标线程自身能看到官方工具为准。",
            ),
            legacy_thread_messenger_evidence,
            [str(paths.config_path), str(paths.database_path)],
        )
    if actionable_initial_session_meta_issue_count or initial_session_meta_scan_error:
        initial_meta_paths = [
            str(item.get("rolloutPath"))
            for item in official_thread_tool_initial_meta.get("sampleCandidates") or []
            if item.get("rolloutPath")
        ]
        add_issue(
            "plugins.official_thread_tools_initial_metadata_drift",
            "warning",
            "plugins",
            localize(language, "Old threads restore incomplete official tool metadata", "旧线程恢复时使用的官方工具元数据不完整"),
            localize(
                language,
                "Some old rollouts have complete SQLite tool rows but their initial session_meta lacks list_threads/read_thread/send_message_to_thread. Appending metadata at the end does not repair restore behavior.",
                "部分旧线程的 SQLite 工具记录完整，但首个 session_meta 缺少 list_threads/read_thread/send_message_to_thread；在 JSONL 尾部追加元数据不能修复恢复行为。",
            ),
            localize(
                language,
                "Run the official-thread-tools repair. It rewrites only initial session_meta records after a full backup and verifies that all user prompts remain byte-order equivalent by count and fingerprint.",
                "运行官方线程工具修复。它会在完整备份后只重写开头的 session_meta，并用数量和指纹验证所有用户 prompt 的顺序与内容保持不变。",
            ),
            legacy_thread_messenger_evidence,
            initial_meta_paths,
        )
    if shared_rollout_alias_count:
        blocked_meta_paths = [
            str(item.get("rolloutPath"))
            for item in official_thread_tool_initial_meta.get("sampleBlockedCandidates") or []
            if item.get("rolloutPath") and item.get("reason") == "shared_rollout_alias"
        ]
        add_issue(
            "plugins.official_thread_tools_shared_rollout_aliases",
            "warning",
            "plugins",
            localize(language, "Thread records share another thread's rollout", "线程记录共用了其它线程的 rollout"),
            localize(
                language,
                "Some SQLite thread ids point to a rollout whose initial session_meta belongs to another thread. Rewriting the JSONL id would corrupt the canonical conversation, so these aliases are deliberately blocked.",
                "部分 SQLite 线程 ID 指向了首个 session_meta 属于其它线程的 rollout。直接重写 JSONL 身份会破坏真实对话，因此这些别名会被明确阻断。",
            ),
            localize(
                language,
                "Inspect the shared thread records and canonical rollout before changing SQLite. Do not use initial metadata repair to force the alias id into the file.",
                "先核对共享线程记录和真实 rollout，再决定如何清理 SQLite；不要用开头元数据修复把别名 ID 强行写进文件。",
            ),
            legacy_thread_messenger_evidence,
            blocked_meta_paths,
        )
    if session_meta_id_mismatch_count:
        mismatch_meta_paths = [
            str(item.get("rolloutPath"))
            for item in official_thread_tool_initial_meta.get("sampleBlockedCandidates") or []
            if item.get("rolloutPath") and item.get("reason") == "session_meta_id_mismatch"
        ]
        add_issue(
            "plugins.official_thread_tools_session_meta_id_mismatch",
            "warning",
            "plugins",
            localize(language, "Thread and rollout identities do not match", "线程记录与 rollout 身份不一致"),
            localize(
                language,
                "Some thread rows point to a rollout whose initial session_meta id does not match any thread sharing that path. This is an identity mismatch, not evidence of a shared-rollout alias.",
                "部分线程记录指向的 rollout 首个 session_meta ID 与该路径关联的任何线程都不一致。这属于身份不匹配，不能按共享 rollout 别名处理。",
            ),
            localize(
                language,
                "Inspect the SQLite thread row, rollout path, and initial session_meta identity together. Do not rewrite either identity until the canonical thread is proven.",
                "同时核对 SQLite 线程记录、rollout 路径和首个 session_meta 身份；在确认真实主线程前，不要改写任何一侧的 ID。",
            ),
            legacy_thread_messenger_evidence,
            mismatch_meta_paths,
        )
    ...
    add_check(
        ...
        macos_plugin_status,
        localize(language, "The iOS and macOS build plugins are inactive on Windows, with no remote install marker or running MCP process.", "iOS 和 macOS 构建插件在 Windows 上未启用，也没有远程安装标记或运行中的 MCP 进程。")
    ...
    node_repl_effective_environment = node_repl_contract["effectiveEnv"]
    ...
    node_repl_desktop_native_pipe_contract = (
        not node_repl_contract["managed"]["tablePresent"]
        and bool(node_repl_effective_command.strip())
        and bool(str(node_repl_effective_environment.get("NODE_REPL_NODE_PATH") or "").strip())
        and bool(str(node_repl_effective_environment.get("NODE_REPL_NODE_MODULE_DIRS") or "").strip())
        and str(node_repl_effective_environment.get("SKY_CUA_NATIVE_PIPE") or "").strip() == "1"
        and bool(str(node_repl_effective_environment.get("SKY_CUA_NATIVE_PIPE_DIRECTORY") or "").strip())
    )
    node_repl_disable_sandbox_without_desktop_contract = (
        node_repl_config_has_disable_sandbox or node_repl_running_with_disable_sandbox > 0
    ) and not node_repl_desktop_native_pipe_contract
    ...
    add_check(
        ...
        [
            ...
            f"desktop_native_pipe_contract={node_repl_desktop_native_pipe_contract}",
