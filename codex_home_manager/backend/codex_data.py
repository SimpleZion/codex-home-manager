...
snapshot_catalog_cache_lock = threading.RLock()
snapshot_catalog_cache: dict[tuple[str, int, bool], tuple[tuple[tuple[str, int, int], ...], dict[str, Any]]] = {}
...
def snapshot_catalog_fingerprint(paths: CodexPaths) -> tuple[tuple[str, int, int], ...]:
    fingerprint: list[tuple[str, int, int]] = []
    for path in (
        paths.database_path,
        paths.database_path.with_name(paths.database_path.name + "-wal"),
        paths.database_path.with_name(paths.database_path.name + "-shm"),
        paths.global_state_path,
        paths.global_state_backup_path,
...
def build_snapshot_catalog(
    ...
    paths = resolve_codex_paths(codex_home_text)
    cache_key = (str(paths.codex_home_path).casefold(), sidebar_limit, validate_rollout_display)
    fingerprint = snapshot_catalog_fingerprint(paths)
    with snapshot_catalog_cache_lock:
        cached = snapshot_catalog_cache.get(cache_key)
        if not force_refresh and cached and cached[0] == fingerprint:
            return cached[1]
    snapshot = build_snapshot(codex_home_text, sidebar_limit, validate_rollout_display)
    snapshot["catalogVersion"] = hashlib.sha256(
        json.dumps(fingerprint, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    ).hexdigest()[:24]
    with snapshot_catalog_cache_lock:
        snapshot_catalog_cache[cache_key] = (fingerprint, snapshot)
    return snapshot


def clear_snapshot_catalog_cache() -> None:
    with snapshot_catalog_cache_lock:
        snapshot_catalog_cache.clear()


def snapshot_thread_matches_status(thread: dict[str, Any], filter_mode: str, thread_kind: str) -> bool:
    if filter_mode == "visible":
        if thread_kind == "subagent":
            return thread.get("threadKind") == "subagent" and not thread.get("archived") and thread.get("fileExists")
        if thread_kind == "all":
            return bool(thread.get("codexVisible")) or (
                thread.get("threadKind") == "subagent" and not thread.get("archived") and thread.get("fileExists")
            )
        return bool(thread.get("codexVisible"))
    if filter_mode == "manual_hidden":
        return thread.get("visibility") == "hidden"
    if filter_mode == "repair":
        return thread.get("visibility") in {"needs_user_event_repair", "missing_file"}
    if filter_mode == "archived":
        return bool(thread.get("archived"))
    return True


def snapshot_thread_search_text(thread: dict[str, Any]) -> str:
    return " ".join(
...
def snapshot_thread_sort_key(thread: dict[str, Any], sort_mode: str) -> tuple[Any, ...]:
    rank = int(thread.get("recentRank") or 999999)
    title = str(thread.get("title") or "").casefold()
    if sort_mode == "status":
        status_order = {
            "visible": 0,
        ...
        primary: Any = status_order.get(str(thread.get("visibility") or ""), 99)
    elif sort_mode == "title":
        primary = title
    elif sort_mode == "project":
        primary = (str(thread.get("projectLabel") or "").casefold(), title)
    elif sort_mode == "size":
        primary = int(thread.get("totalFileSizeBytes") or 0)
    elif sort_mode == "tokens":
        primary = int(thread.get("totalTokensUsed") or 0)
    elif sort_mode == "updated":
        primary = int(thread.get("updatedAtMs") or 0)
    else:
        primary = rank
    return primary, rank, title, str(thread.get("id") or "")


def truncate_snapshot_display_text(value: Any, limit: int) -> str:
    text = str(value or "")
    if len(text) <= limit:
        return text
...
def snapshot_thread_page_record(thread: dict[str, Any]) -> dict[str, Any]:
    record = dict(thread)
    for key, limit in {
        "title": 500,
        "sqliteTitle": 500,
        "sidebarTitle": 500,
        "sessionIndexTitle": 500,
        "rolloutTitle": 500,
        "preview": 1200,
        "source": 500,
    }.items():
        record[key] = truncate_snapshot_display_text(record.get(key), limit)
    record["hiddenReasons"] = [
        truncate_snapshot_display_text(reason, 240)
        for reason in list(record.get("hiddenReasons") or [])[:24]
    ]
    return record


def snapshot_page_cursor(offset: int, query_hash: str, catalog_version: str) -> str:
    payload = json.dumps(
        {"offset": offset, "query": query_hash, "catalog": catalog_version},
        separators=(",", ":"),
    ).encode("utf-8")
    return base64.urlsafe_b64encode(payload).decode("ascii").rstrip("=")


def decode_snapshot_page_cursor(cursor: str, query_hash: str, catalog_version: str) -> int:
    try:
        padded = cursor + "=" * (-len(cursor) % 4)
        payload = json.loads(base64.urlsafe_b64decode(padded).decode("utf-8"))
        offset = int(payload["offset"])
    except (ValueError, KeyError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError("invalid thread page cursor") from error
    if payload.get("query") != query_hash or payload.get("catalog") != catalog_version or offset < 0:
        raise ValueError("thread page cursor does not match the current query or catalog")
    return offset


def query_snapshot_thread_page(
    codex_home_text: str | None = None,
    sidebar_limit: int = 50,
    thread_kind: str = "main",
    filter_mode: str = "all",
    project_path: str = "all",
    search: str = "",
    sort_mode: str = "rank",
    ...
    snapshot = build_snapshot_catalog(
        codex_home_text=codex_home_text,
        sidebar_limit=sidebar_limit,
    ...
    query_hash = hashlib.sha256(
        json.dumps(query_shape, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()[:24]
    catalog_version = str(snapshot["catalogVersion"])
    offset = decode_snapshot_page_cursor(cursor, query_hash, catalog_version) if cursor else 0

    scope_threads = [
        thread for thread in snapshot["threads"]
        if (thread_kind == "all" or thread.get("threadKind") == thread_kind)
        and snapshot_thread_matches_status(thread, filter_mode, thread_kind)
    ]
    project_counts: dict[str, int] = {}
    for thread in scope_threads:
        path = str(thread.get("projectPath") or "")
        project_counts[path] = project_counts.get(path, 0) + 1

    matched_threads = [
        thread for thread in scope_threads
        if (project_path == "all" or str(thread.get("projectPath") or "") == project_path)
        and (not normalized_search or normalized_search in snapshot_thread_search_text(thread))
    ]
    reverse = sort_direction == "desc"
    matched_threads.sort(key=lambda thread: snapshot_thread_sort_key(thread, sort_mode), reverse=reverse)
    page_threads = matched_threads[offset : offset + limit]
    next_offset = offset + len(page_threads)
    has_more = next_offset < len(matched_threads)
    return {
        "codexHome": snapshot["codexHome"],
        "catalogVersion": catalog_version,
        "generatedAtMs": snapshot["generatedAtMs"],
        "matchedThreads": len(matched_threads),
        "loadedThreads": len(page_threads),
        "threads": [snapshot_thread_page_record(thread) for thread in page_threads],
        "projectCounts": project_counts,
        "hasMore": has_more,
        "nextCursor": snapshot_page_cursor(next_offset, query_hash, catalog_version) if has_more else None,
    }
...
def build_snapshot_summary(
    ...
    snapshot = build_snapshot_catalog(codex_home_text, sidebar_limit, force_refresh=force_refresh)
    response = {
        key: value for key, value in snapshot.items()
        if key != "threads"
    }
    response["statusCounts"] = {
        status: sum(1 for thread in snapshot["threads"] if thread.get("visibility") == status)
        for status in (
            "visible",
    ...
    response["threadHighlights"] = [
        snapshot_thread_page_record(thread)
        for thread in sorted(
            snapshot["threads"],
            key=lambda thread: (
                int(thread.get("fileSizeBytes") or 0),
                int(thread.get("tokensUsed") or 0),
            ),
            reverse=True,
        )[:4]
    ]
