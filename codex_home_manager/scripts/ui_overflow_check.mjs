...
async function measureViewport(page, viewport, label) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    const actionButtons = Array.from(document.querySelectorAll(".thread-table .row-action-button"));
    ...
    const actionButtonMetrics = actionButtons.map((button) => {
      ...
      return {
        ...
        insideCell: Boolean(cellRect && buttonRect.left >= cellRect.left - 1 && buttonRect.right <= cellRect.right + 1),
        insideWrap: Boolean(wrapRect && buttonRect.left >= wrapRect.left - 1 && buttonRect.right <= wrapRect.right + 1),
        insidePage: buttonRect.left >= -1 && buttonRect.right <= document.documentElement.clientWidth + 1
      };
    });
    ...
    return {
      ...
      actionVisibleInsideWrap: firstActionCell
        ...
        : true,
      rowActionButtonCount: actionButtonMetrics.length,
      rowActionButtonMinWidth: actionButtonMetrics.length ? Math.min(...actionButtonMetrics.map((button) => button.width)) : null,
      actionButtonsInsideCells: actionButtonMetrics.every((button) => button.insideCell),
      actionButtonsInsideWrap: actionButtonMetrics.every((button) => button.insideWrap),
      actionButtonsInsidePage: actionButtonMetrics.every((button) => button.insidePage)
    };
  });
  ...
  assertCondition(metrics.rowActionButtonCount > 0, `${label} thread action buttons exist`);
  assertCondition(metrics.rowActionButtonMinWidth >= 60, `${label} thread action buttons keep a practical width`);
  assertCondition(metrics.actionButtonsInsideCells, `${label} thread action buttons stay inside operation cells`);
  assertCondition(metrics.actionButtonsInsideWrap, `${label} thread action buttons stay inside table viewport`);
  assertCondition(metrics.actionButtonsInsidePage, `${label} thread action buttons stay inside page viewport`);
