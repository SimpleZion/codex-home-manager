...
function Get-VerifiedGithubRelease {
    ...
    $verifierPath = Join-Path $publicSiteDirectory "codex-home-manager-verifier-win-x64.exe"
    if (-not (Test-Path -LiteralPath $verifierPath -PathType Leaf)) {
        throw "Self-contained release verifier is missing: $verifierPath"
    }
    $verifierMetadata = [pscustomobject]@{
        name = "codex-home-manager-verifier-win-x64.exe"
        size = (Get-Item -LiteralPath $verifierPath).Length
        sha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $verifierPath).Hash.ToLowerInvariant()
    }
    $artifactMetadata = @($connectorArtifactMetadata) + @($verifierMetadata) + @($sourceEvidenceAssets)
