...
embedded_release_public_key_fingerprint = "sha256:ef7194fbc8fa8550430c908d9d02c74f7fc0d1e87f7f9b4ec5a164526b48f208"
...
verification_public_names = (
    "codex-home-manager-verifier-win-x64.exe",
    "verify-codex-home-manager.ps1",
    release_public_key_name,
    release_fingerprint_name,
)
...
user_artifact_verifier_source = Path(__file__).with_name("artifact_verifier_core.py").read_text(encoding="utf-8")
...
def render_powershell_artifact_verifier(*, default_artifact_name: str, trusted_fingerprint: str) -> str:
    if Path(default_artifact_name).name != default_artifact_name or re.fullmatch(
        r"codex-home-manager-local-win-x64-v[0-9]+\.[0-9]+\.[0-9]+-[0-9a-f]{12}\.(exe|zip)",
        default_artifact_name,
    ) is None:
        raise ReleaseManifestError("default verifier artifact name is invalid")
    normalized_fingerprint = normalize_fingerprint(trusted_fingerprint)
    if normalized_fingerprint != embedded_release_public_key_fingerprint:
        raise ReleaseManifestError("verifier trust anchor differs from the independently pinned release key")
...
$trustedPublicKeyFingerprint = "__TRUSTED_FINGERPRINT__"
$defaultArtifactName = "__DEFAULT_ARTIFACT_NAME__"
$pythonVerifierSource = @'
__PYTHON_VERIFIER_SOURCE__'@
...
'''
    return (
        template.replace("__TRUSTED_FINGERPRINT__", normalized_fingerprint)
        .replace("__DEFAULT_ARTIFACT_NAME__", default_artifact_name)
        .replace("__PYTHON_VERIFIER_SOURCE__", user_artifact_verifier_source)
    )


def write_powershell_artifact_verifier(
    output_path: Path, *, default_artifact_name: str, trusted_fingerprint: str
) -> None:
    write_bytes(
        output_path,
        render_powershell_artifact_verifier(
...
def validate_authenticode_evidence(value: Any) -> None:
    ...
    self_signed = value.get("selfSigned")
    chain_trusted = value.get("chainTrusted")
    has_signer = isinstance(thumbprint, str) and re.fullmatch(r"[0-9A-F]{40}", thumbprint) is not None
...
def validate_public_bundle_metadata(bundle: Any) -> list[dict[str, Any]]:
    if not isinstance(bundle, dict):
        raise ReleaseManifestError("public artifact bundle is not a JSON object")
    if bundle.get("schemaVersion") != 2 or not isinstance(bundle.get("version"), str):
...
def load_public_bundle(site_directory: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    artifacts = validate_public_bundle_metadata(bundle)
...
def collect_public_artifact_records(
    ...
    for name in verification_public_names:
        path = site_directory / name
        record = record_for_path(path)
        if checksum_entries.get(name) != record["sha256"]:
...
def validate_github_release_evidence(
    ...
    verifier_path = public_site_directory / verification_public_names[0]
    expected_artifacts[verifier_path.name] = record_for_path(verifier_path)
...
def verify_online_release(
    *,
    ...
    if not isinstance(github_artifact_records, list) or len(github_artifact_records) != 3 + len(source_evidence_public_names):
    ...
    product_asset_names = github_artifact_names - set(source_evidence_public_names)
    verifier_name = verification_public_names[0]
    if verifier_name not in product_asset_names:
        raise ReleaseManifestError("online release GitHub artifact proof has no self-contained verifier")
    connector_names = product_asset_names - {verifier_name}
    ...
    signed_public_records = manifest.get("public_artifacts")
    if not isinstance(signed_public_records, list):
        raise ReleaseManifestError("online release manifest has no public artifacts")
    verifier_public_record = next(
        (
            record
            for record in signed_public_records
            if isinstance(record, dict) and record.get("path") == verifier_name
        ),
        None,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    user_verifier = subparsers.add_parser("write-user-verifier")
    user_verifier.add_argument("--output", required=True, type=path_argument)
    user_verifier.add_argument("--default-artifact-name", required=True)
    user_verifier.add_argument("--trusted-public-key-fingerprint", required=True)
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif parsed.command == "write-user-verifier":
            write_powershell_artifact_verifier(
                parsed.output,
                default_artifact_name=parsed.default_artifact_name,
                trusted_fingerprint=parsed.trusted_public_key_fingerprint,
            )
