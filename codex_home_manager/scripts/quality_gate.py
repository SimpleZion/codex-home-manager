...
def run_static_browser_mode_gate() -> None:
    ...
    assert_condition(
        re.search(r"extractThreadMetadata\(parsed,\s*[A-Za-z_$][\w$]*\)", browser_home_text) is not None,
        "browser folder scan parses thread metadata",
    )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="codex-home-manager-quality-gate-") as temporary_directory:
        try:
            ...
            run_command([sys.executable, "scripts/maintainability_gate.py"])
            run_command([sys.executable, "-m", "pytest", "tests"])
            run_command(["npm", "run", "test:browser-contracts"])
