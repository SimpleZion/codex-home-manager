...
$verifierExecutablePath = Join-Path $releaseRoot "codex-home-manager-verifier-win-x64.exe"
...
$verifierSourcePath = Join-Path $appDirectory "packaging\windows\release_verifier.py"
...
set "APP=%~dp0CodexHomeManagerLocal\CodexHomeManagerLocal.exe"
"%APP%" --status
exit /b %ERRORLEVEL%
'@ | Set-Content -LiteralPath (Join-Path $PackageDirectory "Status Codex Home Manager.cmd") -Encoding ASCII
...
"%APP%" --stop
exit /b %ERRORLEVEL%
'@ | Set-Content -LiteralPath (Join-Path $PackageDirectory "Stop Codex Home Manager.cmd") -Encoding ASCII
...
set "APP=%~dp0CodexHomeManagerLocal\CodexHomeManagerLocal.exe"
"%APP%" --uninstall
echo Connector state stopped and browser protocol removed. Product files and local data were preserved.
pause
'@ | Set-Content -LiteralPath (Join-Path $PackageDirectory "Uninstall Codex Home Manager.cmd") -Encoding ASCII
...
- Download codex-home-manager-verifier-win-x64.exe beside the app and run it to verify the signed release without Python.

ZIP fallback:
1. Run "Start Codex Home Manager.cmd" to start the connector.
2. Use "Status Codex Home Manager.cmd" and "Stop Codex Home Manager.cmd" for lifecycle control.
3. Before upgrading, stop the verified connector, replace the extracted folder, and start the new version.
4. Run "Uninstall Codex Home Manager.cmd" to stop the connector and remove only its browser protocol; local data and backups are preserved.
...
function Invoke-IsolatedConnectorBuild {
    ...
    $verifierRoot = Join-Path $iterationRoot "verifier"
    ...
    $verifierWorkRoot = Join-Path $iterationRoot "work-verifier"
    New-Item -ItemType Directory -Force -Path $iterationRoot, $payloadRoot, $packageRoot, $oneFileRoot, $verifierRoot, $specRoot | Out-Null
    ...
    & $venvPython $pyinstallerRunnerPath `
            --noconfirm --clean --name CodexHomeManagerVerifier --onefile --console `
            --icon $iconPath --version-file $versionInfoPath --distpath $verifierRoot --workpath $verifierWorkRoot --specpath $specRoot `
            --paths (Join-Path $appDirectory "scripts") --paths (Join-Path $appDirectory "packaging\windows") `
            --hidden-import cryptography.hazmat.primitives.asymmetric.ed25519 `
            $verifierSourcePath
    if ($LASTEXITCODE -ne 0) {
        throw "PyInstaller verifier build failed in isolated build $BuildName"
    }
    ...
    $verifierExecutable = Join-Path $verifierRoot "CodexHomeManagerVerifier.exe"
    foreach ($path in @($directExecutable, $oneDirExecutable, $verifierExecutable)) {
...
try {
    ...
    if ($PSCmdlet.ParameterSetName -eq "CiBuild") {
        ...
        if ($FullReleaseValidation -or $VerifyReproducibleBuild) {
            ...
            if ((Get-Sha256HashText -Path $firstBuild.Verifier) -cne (Get-Sha256HashText -Path $secondBuild.Verifier)) {
                throw "Two isolated verifier builds were not byte-for-byte reproducible"
            }
        }
    ...
    & python (Join-Path $PSScriptRoot "export_public_api_contract.py") `
        --output (Join-Path $publicSiteRoot "public-api.json")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to export the runtime-aligned public REST and MCP contract"
    }
    ...
    $checksums = @($directExecutablePath, $archivePath, $verifierExecutablePath, $verifyScriptPath) | ForEach-Object {
    ...
    Copy-Item -LiteralPath $verifierExecutablePath -Destination (Join-Path $publicSiteRoot "codex-home-manager-verifier-win-x64.exe") -Force
    ...
    $publicChecksumPaths = @(
        ...
        $publicArchivePath,
        (Join-Path $publicSiteRoot "connector-release.json"),
        (Join-Path $publicSiteRoot "codex-home-manager-verifier-win-x64.exe"),
...
"@
    ...
    Write-Output (Join-Path $stableReleaseRoot "codex-home-manager-verifier-win-x64.exe")
