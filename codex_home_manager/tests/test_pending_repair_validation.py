...
def test_pending_repair_lock_follows_configured_backup_root(monkeypatch, tmp_path: Path) -> None:
    backup_root = tmp_path / "configured" / "codex_home_manager"
    monkeypatch.setenv("CODEX_HOME_MANAGER_BACKUP_ROOT", str(backup_root))

    assert pending_repair_validation.pending_repair_lock_path() == (
        backup_root.parent / "codex_full_repair" / "active_repair.lock.json"
    ).resolve()
