...
const promptClassificationPath = path.join(projectRoot, "src", "promptClassification.ts");
const promptClassificationSource = fs.readFileSync(promptClassificationPath, "utf8");
const promptClassificationModule = { exports: {} };
const promptClassificationJavaScript = ts.transpileModule(promptClassificationSource, {
  compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 }
}).outputText;
new Function("exports", "module", promptClassificationJavaScript)(
  promptClassificationModule.exports,
  promptClassificationModule
);
const { classifyPromptText } = promptClassificationModule.exports;

const recommendedPluginsClassification = classifyPromptText(
  "<recommended_plugins>\n- Sentry\n</recommended_plugins>\n# AGENTS.md instructions\n<environment_context>...</environment_context>"
);
assert.equal(recommendedPluginsClassification.sourceType, "internal");
assert.equal(recommendedPluginsClassification.sourceLabel, "推荐插件上下文");
assert.equal(recommendedPluginsClassification.visibleByDefault, false);
assert.equal(recommendedPluginsClassification.pureText, "");
assert.equal(recommendedPluginsClassification.hasPureText, false);
assert.equal(classifyPromptText("<skills_instructions>runtime</skills_instructions>").sourceLabel, "运行时上下文");
assert.equal(
  classifyPromptText("请解释正文中的 <recommended_plugins> 标签").sourceType,
  "user",
  "runtime wrappers mentioned inside real user prose must not be hidden"
);
  ...
  try {
    ...
    await page.route("**/*", async (route) => {
      ...
      if (pathname === "/api/snapshot/summary") {
        const { threads: _threads, ...summary } = snapshotPayload();
        await route.fulfill({ json: { ...summary, catalogVersion: "test-catalog", statusCounts: {}, threadHighlights: [] } });
        return;
      }
      if (pathname === "/api/threads/page") {
        await route.fulfill({ json: { codexHome: "D:/Codex", catalogVersion: "test-catalog", generatedAtMs: Date.now(), matchedThreads: 0, loadedThreads: 0, threads: [], projectCounts: {}, hasMore: false, nextCursor: null } });
        return;
      }
...
async function runHostedBehaviorTest() {
  ...
  try {
    ...
    assert.ok(
      localApiRequests.some((url) => url === "http://127.0.0.1:8765/api/capabilities"),
      `hosted startup must probe only the public connector capability endpoint: ${localApiRequests.join(" | ")}`
    );
    assert.ok(
      localApiRequests.every((url) => url === "http://127.0.0.1:8765/api/capabilities"),
      `hosted startup must not read private local data before redirecting to local mode: ${localApiRequests.join(" | ")}`
    );
