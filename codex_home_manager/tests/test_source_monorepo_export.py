...
def source_fixture(tmp_path: Path) -> tuple[Path, Path]:
    root_repository = create_repository(
        tmp_path / "root",
        {
            ...
            ".gitignore": "private/\n",
            ".source-publication.gitignore": "__pycache__/\n*.pyc\n",
...
def test_export_contains_only_monorepo_sources_and_commit_manifest(tmp_path: Path) -> None:
    ...
    assert result["managerFiles"] == 17
    assert (output / "README.md").read_text(encoding="utf-8") == "# Product quick start\n"
    assert (output / "LICENSE").read_text(encoding="utf-8") == "MIT License\n"
    assert (output / "CONTRIBUTING.md").is_file()
    assert (output / "docs" / "SOURCE_EXPORT.md").read_text(encoding="utf-8") == "# Exported source\n"
    ...
    tracked_files = {
        path
        for path in run_git(output, "ls-files").splitlines()
        if path
    }
