...
from backend.codex_data import (
    ...
    inspect_official_thread_tool_registry,
    inspect_official_thread_tool_initial_session_meta,
    ...
    preview_portable_backup_restore,
    ...
    write_portable_backup_manifest,
...
def test_parse_mcp_process_snapshot_ignores_node_repl_kernel_asset_path() -> None:
    snapshot = parse_mcp_process_snapshot(
        [
            {
                "ProcessId": 7,
                "ParentProcessId": 1,
                "Name": "node_repl.exe",
                "CommandLine": r'"C:\runtime\node_repl.exe" --disable-sandbox',
            },
            {
                "ProcessId": 8,
                "ParentProcessId": 7,
                "Name": "node.exe",
                "CommandLine": r'"C:\runtime\node.exe" D:\.codex\node_repl\kernel_assets\kernel.js',
            },
        ]
    )

    assert snapshot["nodeReplProcessCount"] == 1
    assert snapshot["nodeReplProcesses"][0]["pid"] == "7"


def test_parse_mcp_process_snapshot_does_not_infer_node_repl_risk_from_disable_sandbox() -> None:
    snapshot = parse_mcp_process_snapshot(
        [
            {
                "ProcessId": 7,
    ...
    assert snapshot["riskProcessCount"] == 0
    assert snapshot["nodeReplRiskProcessCount"] == 0
    assert snapshot["nodeReplWithDisableSandboxCount"] == 1
...
def test_snapshot_summary_and_thread_page_are_bounded_for_thousand_thread_home(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    shared_rollout_path = codex_home_path / "sessions" / "rollout-thread-0.jsonl"
    with closing(sqlite3.connect(codex_home_path / "state_5.sqlite")) as connection, connection:
        rows = []
        for index in range(3, 1203):
            rows.append(
                (
                    f"thread-{index}",
                    str(shared_rollout_path),
                    100 + index,
                    100 + index,
        ...
        connection.executemany(
            ...
            INSERT INTO threads (
                ...
                sandbox_policy, approval_mode, tokens_used, has_user_event, archived,
                cli_version, first_user_message, memory_mode, model, created_at_ms, updated_at_ms, preview
            ) VALUES (?, ?, ?, ?, 'test', 'openai', ?, ?, '{}', 'never', ?, 1, 0, 'test', 'hello', 'enabled', 'gpt', ?, ?, ?)
            """,
            rows,
        )

    codex_data_module.clear_snapshot_catalog_cache()
    client = TestClient(server.app, base_url="http://127.0.0.1:8876")
    started_at = time.perf_counter()
    summary_response = client.get(
        "/api/snapshot/summary",
        params={"codex_home": str(codex_home_path), "sidebar_limit": 50},
    )
    summary_seconds = time.perf_counter() - started_at
    assert summary_response.status_code == 200
    assert summary_response.json()["summary"]["totalThreads"] == 1203
    assert len(summary_response.content) < 100_000
    assert summary_seconds < 3.0
    ...
    page_seconds = time.perf_counter() - started_at
    assert first_page_response.status_code == 200
    page = first_page_response.json()
    assert page["matchedThreads"] == 1203
    assert len(page["threads"]) == 50
    assert page["hasMore"] is True
    assert page["nextCursor"]
    assert len(first_page_response.content) < 250_000
    ...
    assert len(second_page_response.json()["threads"]) == 50
    assert {thread["id"] for thread in page["threads"]}.isdisjoint(
        {thread["id"] for thread in second_page_response.json()["threads"]}
    )
...
def test_read_thread_prompts_supports_event_only_and_deduplicates_cross_protocol_mirrors(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {
        ...
        },
        {
            ...
            "payload": {"type": "user_message", "message": "mirrored prompt"},
        },
        {
            "type": "response_item",
            "timestamp": "2026-08-13T01:01:00.250Z",
            "payload": {
                "type": "message",
                "role": "user",
                "content": [{"type": "input_text", "text": "mirrored prompt"}],
            },
        },
        {
            "type": "user_message",
            "timestamp": "2026-08-13T01:01:00.500Z",
            "payload": {"text": "mirrored prompt"},
        },
        {
            "type": "response_item",
    ...
    assert [prompt["protocol"] for prompt in result["prompts"][1:]] == [
        "event_msg",
        "event_msg",
        "response_item",
    ]
...
def test_prompt_index_cache_invalidates_when_rollout_changes(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    first = read_thread_prompts(str(codex_home_path), "thread-1")
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    with rollout_path.open("a", encoding="utf-8") as output:
        output.write(
            json.dumps(
                {
                    ...
                    "payload": {"type": "user_message", "message": "new event prompt"},
                },
                ensure_ascii=False,
            )
            + "\n"
    ...
    second = read_thread_prompts(str(codex_home_path), "thread-1")

    assert first["promptCount"] == 1
    assert second["promptCount"] == 2
    assert second["prompts"][-1]["text"] == "new event prompt"
...
def test_read_thread_prompts_excludes_codex_runtime_context_from_pure_user_text(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    runtime_prompts = [
        (
            ...
            "<environment_context><cwd>C:\\workspace</cwd></environment_context>"
        ),
        "<skills_instructions>\nRuntime skill inventory\n</skills_instructions>",
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for offset, text in enumerate(runtime_prompts, 1):
            output.write(
                json.dumps(
                    {
                        "type": "response_item",
                        "timestamp": f"2026-08-12T00:0{offset}:00Z",
                        "payload": {
                            "type": "message",
                            "role": "user",
            ...
            json.dumps(
                {
                    ...
                    "payload": {
                        ...
                        "content": [
                            {
                                "type": "input_text",
                                "text": "请解释正文里提到的 <recommended_plugins> 标签。",
                            }
                        ],
                    },
                }
    ...
    assert result["sourceCounts"] == {"user": 2, "internal": 2}
    recommended_prompt = result["prompts"][1]
    skills_prompt = result["prompts"][2]
    user_prompt = result["prompts"][3]
    assert recommended_prompt["sourceType"] == "internal"
    assert recommended_prompt["sourceLabel"] == "推荐插件上下文"
    assert recommended_prompt["visibleByDefault"] is False
    assert recommended_prompt["pureText"] == ""
    assert recommended_prompt["hasPureText"] is False
    assert skills_prompt["sourceLabel"] == "运行时上下文"
    assert skills_prompt["pureText"] == ""
    assert user_prompt["sourceType"] == "user"
    assert user_prompt["pureText"] == "请解释正文里提到的 <recommended_plugins> 标签。"

    pure_export = export_thread_prompts(str(codex_home_path), "thread-1")
    pure_export_text = Path(pure_export["outputPath"]).read_text(encoding="utf-8")
    all_export = export_thread_prompts(str(codex_home_path), "thread-1", scope="all")
    all_export_text = Path(all_export["outputPath"]).read_text(encoding="utf-8")
    assert pure_export["promptCount"] == 2
    assert "Here is a list of plugins that are available but not installed" not in pure_export_text
    assert "Runtime skill inventory" not in pure_export_text
    assert "请解释正文里提到的 <recommended_plugins> 标签。" in pure_export_text
    assert all_export["promptCount"] == 4
    assert "Here is a list of plugins that are available but not installed" in all_export_text
    assert "Runtime skill inventory" in all_export_text
...
def test_thread_timeline_normalizes_deduplicates_and_pages_from_file_tail(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {"type": "event_msg", "timestamp": "2026-08-12T01:00:00Z", "payload": {"type": "agent_reasoning", "text": "**检查数据协议**"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:01Z", "payload": {"type": "reasoning", "id": "rs_1", "summary": [], "encrypted_content": "encrypted"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:02Z", "payload": {"type": "custom_tool_call", "name": "exec", "call_id": "call-1", "input": "echo ok"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:03Z", "payload": {"type": "custom_tool_call_output", "call_id": "call-1", "output": [{"type": "input_text", "text": "ok"}]}},
        {"type": "event_msg", "timestamp": "2026-08-12T01:00:04Z", "payload": {"type": "agent_message", "message": "修复完成", "phase": "final"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:05Z", "payload": {"type": "message", "role": "assistant", "phase": "final", "content": [{"type": "output_text", "text": "修复完成\n\n附加元信息"}]}},
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for record in records:
            output.write(json.dumps(record, ensure_ascii=False) + "\n")

    first_page = read_thread_timeline(str(codex_home_path), "thread-1", limit=3, kind_filter="all")

    assert first_page["hasMore"] is True
    assert first_page["nextBeforeByte"] is not None
    assert len(first_page["items"]) == 3
    assert [item["kind"] for item in first_page["items"]] == ["tool_call", "tool_output", "assistant"]
    assert first_page["items"][-1]["text"] == "修复完成\n\n附加元信息"
    assert first_page["scannedRecords"] < 12

    older_page = read_thread_timeline(
        str(codex_home_path),
        "thread-1",
        before_byte=first_page["nextBeforeByte"],
        limit=10,
        kind_filter="all",
    )
    reasoning_items = [item for item in older_page["items"] if item["kind"] == "reasoning"]
    assert len(reasoning_items) == 2
    assert reasoning_items[0]["readable"] is True
    assert reasoning_items[0]["text"] == "**检查数据协议**"
    assert reasoning_items[0]["label"] == "推理记录"
    assert reasoning_items[1]["encrypted"] is True
    assert reasoning_items[1]["readable"] is False
    assert reasoning_items[1]["text"] == ""
    assert reasoning_items[1]["label"] == "推理记录"


def test_thread_timeline_separates_progress_commentary_from_final_replies_and_tools(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {"type": "response_item", "timestamp": "2026-08-12T01:00:00Z", "payload": {"type": "message", "role": "user", "content": [{"type": "input_text", "text": "开始"}]}},
        {"type": "event_msg", "timestamp": "2026-08-12T01:00:01Z", "payload": {"type": "agent_message", "phase": "commentary", "message": "正在验证构建顺序"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:01Z", "payload": {"type": "message", "role": "assistant", "phase": "commentary", "content": [{"type": "output_text", "text": "正在验证构建顺序"}]}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:02Z", "payload": {"type": "custom_tool_call", "name": "exec", "call_id": "call-1", "input": "npm test"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:03Z", "payload": {"type": "message", "role": "assistant", "phase": "final_answer", "content": [{"type": "output_text", "text": "验证完成"}]}},
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for record in records:
            output.write(json.dumps(record, ensure_ascii=False) + "\n")

    main_page = read_thread_timeline(str(codex_home_path), "thread-1", kind_filter="conversation", limit=20)
    commentary_page = read_thread_timeline(str(codex_home_path), "thread-1", kind_filter="commentary", limit=20)
    final_page = read_thread_timeline(str(codex_home_path), "thread-1", kind_filter="assistant", limit=20)

    assert [item["kind"] for item in main_page["items"]][-3:] == ["user", "commentary", "assistant"]
    assert not any(item["kind"].startswith("tool") for item in main_page["items"])
    assert [item["text"] for item in commentary_page["items"]] == ["正在验证构建顺序"]
    assert commentary_page["items"][0]["label"] == "思考过程"
    assert [item["text"] for item in final_page["items"]] == ["验证完成"]
    assert final_page["items"][-1]["label"] == "最终回复"


def test_thread_timeline_preserves_real_repeats_and_deduplicates_across_page_boundary(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {"type": "response_item", "timestamp": "2026-08-12T01:00:00Z", "payload": {"type": "message", "role": "user", "content": [{"type": "input_text", "text": "继续"}]}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:01Z", "payload": {"type": "message", "role": "user", "content": [{"type": "input_text", "text": "继续"}]}},
        {"type": "event_msg", "timestamp": "2026-08-12T01:00:02Z", "payload": {"type": "agent_message", "message": "完成"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:03Z", "payload": {"type": "message", "role": "assistant", "content": [{"type": "output_text", "text": "完成并验证"}]}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:04Z", "payload": {"type": "message", "role": "user", "content": [{"type": "input_text", "text": "下一步"}]}},
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for record in records:
            output.write(json.dumps(record, ensure_ascii=False) + "\n")

    first_page = read_thread_timeline(str(codex_home_path), "thread-1", limit=2)
    second_page = read_thread_timeline(
        str(codex_home_path),
        "thread-1",
        before_byte=first_page["nextBeforeByte"],
        limit=10,
    )
    combined = [*second_page["items"], *first_page["items"]]

    assert [item["text"] for item in combined].count("完成") == 0
    assert [item["text"] for item in combined].count("完成并验证") == 1
    assert [item["text"] for item in combined].count("继续") == 2


def test_thread_timeline_classifies_runtime_context_and_redacts_attachment_payloads(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {"type": "user_message", "timestamp": "2026-08-12T01:00:00Z", "payload": "legacy prompt"},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:01Z", "payload": {"type": "message", "role": "user", "content": [{"type": "input_text", "text": "<recommended_plugins>runtime</recommended_plugins>"}]}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:02Z", "payload": {"type": "custom_tool_call", "name": "empty_tool", "call_id": "empty-call", "input": ""}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:03Z", "payload": {"type": "custom_tool_call_output", "call_id": "image-call", "output": '{"image_url":"data:image/png;base64,SECRET"}'}},
        {"type": "event_msg", "timestamp": "2026-08-12T01:00:04Z", "payload": {"type": "patch_apply_end", "call_id": "patch-1"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:05Z", "payload": {"type": "custom_tool_call_output", "call_id": "embedded-image", "output": "preview=data:image/png;base64,PRIVATE_BYTES_123456"}},
        {"type": "response_item", "timestamp": "2026-08-12T01:00:06Z", "payload": {"type": "custom_tool_call_output", "call_id": "short-image", "output": "preview=data:image/png;base64,YQ=="}},
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for record in records:
            output.write(json.dumps(record, ensure_ascii=False) + "\n")

    user_page = read_thread_timeline(str(codex_home_path), "thread-1", kind_filter="user", limit=20)
    all_page = read_thread_timeline(str(codex_home_path), "thread-1", kind_filter="all", limit=20)

    assert any(item["text"] == "legacy prompt" for item in user_page["items"])
    assert not any("recommended_plugins" in item["text"] for item in user_page["items"])
    runtime_item = next(item for item in all_page["items"] if "recommended_plugins" in item["text"])
    assert runtime_item["kind"] == "context"
    assert runtime_item["promptSourceType"] == "internal"
    assert any(item["text"] == "[无参数]" for item in all_page["items"])
    assert any(item["label"] == "patch-1" and item["kind"] == "tool_output" for item in all_page["items"])
    assert "SECRET" not in json.dumps(all_page, ensure_ascii=False)
    assert "PRIVATE_BYTES" not in json.dumps(all_page, ensure_ascii=False)
    assert "YQ==" not in json.dumps(all_page, ensure_ascii=False)
    assert "[图片附件]" in json.dumps(all_page, ensure_ascii=False)
    assert "[附件内容已隐藏]" in json.dumps(all_page, ensure_ascii=False)


def test_thread_timeline_supports_tool_search_and_mixed_encrypted_reasoning(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    records = [
        {"type": "response_item", "payload": {"type": "tool_search_call", "call_id": "search-1", "query": "thread tools"}},
        {"type": "response_item", "payload": {"type": "tool_search_output", "call_id": "search-1", "tools": [{"name": "read_thread"}]}},
        {"type": "response_item", "payload": {"type": "reasoning", "summary": [{"type": "summary_text", "text": "可读摘要"}], "encrypted_content": "opaque"}},
    ]
    with rollout_path.open("a", encoding="utf-8") as output:
        for record in records:
            output.write(json.dumps(record, ensure_ascii=False) + "\n")
    ...
    search_call = next(item for item in page["items"] if item["payloadType"] == "tool_search_call")
    search_output = next(item for item in page["items"] if item["payloadType"] == "tool_search_output")
    reasoning = next(item for item in page["items"] if item["payloadType"] == "reasoning")
    assert search_call["kind"] == "tool_call"
    assert "thread tools" in search_call["text"]
    assert search_output["kind"] == "tool_output"
    assert "read_thread" in search_output["text"]
    assert reasoning["text"] == "可读摘要"
    assert reasoning["readable"] is True
...
def test_thread_timeline_recovers_large_jsonl_record_with_stream_redaction(tmp_path: Path) -> None:
    ...
    with rollout_path.open("a", encoding="utf-8") as output:
        output.write(json.dumps({"type": "compacted", "payload": {"message": oversized_text}}) + "\n")
        output.write(json.dumps({"type": "event_msg", "payload": {"type": "agent_message", "phase": "commentary", "message": "安全读取"}}, ensure_ascii=False) + "\n")
...
def test_reverse_jsonl_reader_bounds_io_for_virtual_1_4_gb_record() -> None:
    ...
    class VirtualPath:
        ...
        def stat(self):
            return type("Stat", (), {"st_size": virtual_size})()
...
def test_thread_timeline_search_stops_at_scan_budget_and_can_continue(tmp_path: Path) -> None:
    ...
    with rollout_path.open("a", encoding="utf-8") as output:
        for index in range(250):
            output.write(json.dumps({"type": "response_item", "payload": {"type": "message", "role": "assistant", "content": [{"type": "output_text", "text": f"entry-{index}"}]}}) + "\n")

    page = read_thread_timeline(
        str(codex_home_path),
        "thread-1",
        search_text="not-present",
        scan_record_limit=100,
    )

    assert page["items"] == []
    assert page["scanLimited"] is True
    assert page["hasMore"] is True
    assert page["nextBeforeByte"] is not None
    assert page["scannedRecords"] == 101


def test_thread_timeline_endpoint_is_read_only_and_searchable(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    with rollout_path.open("a", encoding="utf-8") as output:
        output.write(json.dumps({"type": "event_msg", "payload": {"type": "agent_reasoning", "text": "unique reasoning marker"}}) + "\n")
    before_hash = hashlib.sha256(rollout_path.read_bytes()).hexdigest()
    client = TestClient(server.app)
    headers = local_authorization_headers(client, codex_home_path)
    response = client.get(
        "/api/threads/thread-1/timeline",
        params={"codex_home": str(codex_home_path), "kind": "reasoning", "search": "unique reasoning"},
        headers=headers,
    )
    ...
    assert payload["items"][0]["kind"] == "reasoning"
    assert payload["items"][0]["text"] == "unique reasoning marker"
    assert hashlib.sha256(rollout_path.read_bytes()).hexdigest() == before_hash


def test_mcp_exposes_and_reads_thread_timeline(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-1.jsonl"
    with rollout_path.open("a", encoding="utf-8") as output:
        output.write(json.dumps({"type": "event_msg", "payload": {"type": "agent_reasoning", "text": "mcp timeline marker"}}) + "\n")
    client = TestClient(server.app)
    api_token = mcp_local_authorization(client, codex_home_path)
    tools_payload = mcp_call(client, 991, "tools/list")
    tool_names = {tool["name"] for tool in tools_payload["result"]["tools"]}
    assert {"codex_thread_timeline", "codex_thread_timeline_item"} <= tool_names
    payload = mcp_call(
        client,
        992,
        "tools/call",
        {"name": "codex_thread_timeline", "arguments": {"codexHome": str(codex_home_path), "apiToken": api_token, "threadId": "thread-1", "kind": "reasoning", "search": "mcp timeline"}},
    )
    assert payload["result"]["structuredContent"]["items"][0]["text"] == "mcp timeline marker"
...
def test_read_rollout_thread_logs_reuses_byte_offset_index_between_pages(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    snapshot = build_snapshot(str(codex_home_path), sidebar_limit=10)
    rollout_path = Path(next(thread["rolloutPath"] for thread in snapshot["threads"] if thread["id"] == "thread-1"))
    with rollout_path.open("a", encoding="utf-8") as file:
        for index in range(20):
            file.write(json.dumps({
                "type": "response_item",
                "timestamp": f"2026-06-03T00:01:{index:02d}Z",
                "payload": {"type": "function_call", "name": f"request_{index}", "arguments": {"index": index}},
            }) + "\n")

    codex_data_module.rollout_log_index_cache.clear()
    original_classifier = codex_data_module.classify_rollout_log_line
    classified_lines = 0

    def counting_classifier(line: str, line_number: int) -> dict[str, object]:
        nonlocal classified_lines
        classified_lines += 1
        return original_classifier(line, line_number)

    monkeypatch.setattr(codex_data_module, "classify_rollout_log_line", counting_classifier)
    first_page = read_thread_logs(
        str(codex_home_path), "thread-1", offset=0, limit=5, kind_filter="request", source_filter="rollout"
    )
    assert first_page["matchedEntries"] == 20
    assert classified_lines > len(first_page["entries"])

    classified_lines = 0
    second_page = read_thread_logs(
        str(codex_home_path), "thread-1", offset=5, limit=5, kind_filter="request", source_filter="rollout"
    )
    assert [entry["label"] for entry in second_page["entries"]] == [f"request_{index}" for index in range(5, 10)]
    assert classified_lines == 5

    with rollout_path.open("a", encoding="utf-8") as file:
        file.write(json.dumps({
            "type": "response_item",
            "timestamp": "2026-06-03T00:02:00Z",
            "payload": {"type": "function_call", "name": "request_20", "arguments": {"index": 20}},
        }) + "\n")
    classified_lines = 0
    refreshed_page = read_thread_logs(
        str(codex_home_path), "thread-1", offset=20, limit=5, kind_filter="request", source_filter="rollout"
    )
    assert refreshed_page["matchedEntries"] == 21
    assert refreshed_page["entries"][0]["label"] == "request_20"
    assert classified_lines > 5
...
def test_diagnostics_ignores_memory_writer_context_window_warning(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    create_test_logs_database(codex_home_path, "thread-1")
    with sqlite3.connect(codex_home_path / "logs_2.sqlite") as connection:
        connection.execute(
            ...
            (
                ...
                50,
                "WARN",
                "codex_memories_write::phase1::job::result",
                "Codex ran out of room in the model's context window while writing background memory.",
                "codex_memories_write",
                "memory_writer.rs",
                50,
                "thread-memory-writer",
                "pid:1",
                210,
            ),
        )
...
def test_portable_backup_restores_same_codex_home_with_signed_package(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    backup = show_thread_in_sidebar(str(codex_home_path), "thread-0")["backup"]
    passphrase = "same-home-portable-passphrase"
    portable = make_backup_portable(backup["backupId"], str(codex_home_path), passphrase)
    with closing(sqlite3.connect(codex_home_path / "state_5.sqlite")) as connection, connection:
        connection.execute("UPDATE threads SET title = 'changed' WHERE id = 'thread-0'")
    Path(backup["rowBefore"]["rollout_path"]).write_text("changed rollout", encoding="utf-8")

    preview = preview_portable_backup_restore(
        backup["backupId"], str(codex_home_path), str(codex_home_path), passphrase
    )
    restored = restore_backup(
        backup["backupId"],
        str(codex_home_path),
        create_backup=False,
        portable_passphrase=passphrase,
        source_codex_home_mapping=str(codex_home_path),
    )

    assert portable["integrityStatus"] == "verified_at_export"
    assert preview["sameCodexHome"] is True
    assert preview["mappingRequired"] is False
    assert "verified and restored from immutable portable manifest" in restored["notes"]
    with closing(sqlite3.connect(codex_home_path / "state_5.sqlite")) as connection:
        title = connection.execute("SELECT title FROM threads WHERE id = 'thread-0'").fetchone()[0]
    assert title == backup["rowBefore"]["title"]
    assert Path(backup["rowBefore"]["rollout_path"]).read_text(encoding="utf-8").startswith(
        '{"type":"session_meta"'
    )


def test_foreign_portable_backup_lists_without_original_hmac_key_and_restores_mapped_home(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source_root = tmp_path / "source"
    target_root = tmp_path / "target"
    source_home = create_test_codex_home(source_root)
    target_home = create_test_codex_home(target_root)
    backup = show_thread_in_sidebar(str(source_home), "thread-0")["backup"]
    passphrase = "cross-device-portable-passphrase"
    make_backup_portable(backup["backupId"], str(source_home), passphrase)
    source_backup_directory = tmp_path / "backups" / backup["backupId"]
    foreign_backup_root = tmp_path / "foreign-backups"
    shutil.copytree(source_backup_directory, foreign_backup_root / backup["backupId"])
    (foreign_backup_root / backup["backupId"] / "manifest.json").unlink()
    monkeypatch.setenv("CODEX_HOME_MANAGER_BACKUP_ROOT", str(foreign_backup_root))
    with closing(sqlite3.connect(target_home / "state_5.sqlite")) as connection, connection:
        connection.execute("UPDATE threads SET title = 'target title' WHERE id = 'thread-0'")
    target_rollout = target_home / "sessions" / "rollout-thread-0.jsonl"
    target_rollout.write_text("target rollout", encoding="utf-8")

    foreign_backups = list_backups(str(target_home))
    with pytest.raises(ValueError, match="mapping does not match"):
        preview_portable_backup_restore(
            backup["backupId"], str(target_home), str(target_home), passphrase
        )
    preview = preview_portable_backup_restore(
        backup["backupId"], str(target_home), str(source_home), passphrase
    )
    restored = restore_backup(
        backup["backupId"],
        str(target_home),
        create_backup=False,
        portable_passphrase=passphrase,
        source_codex_home_mapping=str(source_home),
    )

    assert len(foreign_backups) == 1
    assert foreign_backups[0]["foreignPortable"] is True
    assert foreign_backups[0]["integrityStatus"] == "passphrase_verification_required"
    assert preview["mappingRequired"] is True
    assert preview["targetCodexHome"] == str(target_home.resolve(strict=False))
    with closing(sqlite3.connect(target_home / "state_5.sqlite")) as connection:
        restored_row = connection.execute(
            "SELECT title, rollout_path FROM threads WHERE id = 'thread-0'"
        ).fetchone()
    assert restored_row[0] == backup["rowBefore"]["title"]
    assert Path(restored_row[1]).resolve(strict=False) == target_rollout.resolve(strict=False)
    assert target_rollout.read_text(encoding="utf-8").startswith('{"type":"session_meta"')
    assert "verified and restored from immutable portable manifest" in restored["notes"]


def test_portable_backup_restores_signed_resource_directory_and_empty_directories(tmp_path: Path) -> None:
    source_home = create_test_codex_home(tmp_path / "source-resource")
    target_home = create_test_codex_home(tmp_path / "target-resource")
    source_resource = source_home / "memories" / "portable-resource"
    (source_resource / "empty").mkdir(parents=True)
    (source_resource / "nested").mkdir()
    (source_resource / "nested" / "note.txt").write_text("signed resource", encoding="utf-8")
    backup = codex_data_module.backup_codex_resource(
        str(source_home), "memories/portable-resource"
    )["backup"]
    passphrase = "portable-directory-passphrase"
    make_backup_portable(backup["backupId"], str(source_home), passphrase)
    target_resource = target_home / "memories" / "portable-resource"
    target_resource.mkdir(parents=True)
    (target_resource / "old.txt").write_text("replaced", encoding="utf-8")
    ...
    preview_after_restore = preview_portable_backup_restore(
        backup["backupId"], str(target_home), str(source_home), passphrase
    )

    restored_resource = target_resource
    assert (restored_resource / "nested" / "note.txt").read_text(encoding="utf-8") == "signed resource"
    assert (restored_resource / "empty").is_dir()
    assert not (restored_resource / "old.txt").exists()
    assert preview_after_restore["integrityStatus"] == "verified"


def test_portable_backup_rejects_tampered_manifest_and_signed_file(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    backup = show_thread_in_sidebar(str(codex_home_path), "thread-0")["backup"]
    passphrase = "tamper-detection-passphrase"
    portable = make_backup_portable(backup["backupId"], str(codex_home_path), passphrase)
    portable_manifest_path = Path(portable["portableManifestPath"])
    with pytest.raises(ValueError, match="passphrase"):
        preview_portable_backup_restore(
            backup["backupId"],
            str(codex_home_path),
            str(codex_home_path),
            "wrong-portable-passphrase",
        )
    portable_manifest = json.loads(portable_manifest_path.read_text(encoding="utf-8"))
    portable_manifest["restoreManifest"]["action"] = "tampered"
    portable_manifest_path.write_text(json.dumps(portable_manifest), encoding="utf-8")
    ...
    make_backup_portable(backup["backupId"], str(codex_home_path), passphrase)
    Path(backup["rolloutBackupPath"]).write_text("tampered payload", encoding="utf-8")
...
def test_portable_backup_rejects_signed_codex_home_path_traversal(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    backup = show_thread_in_sidebar(str(codex_home_path), "thread-0")["backup"]
    passphrase = "path-traversal-passphrase"
    portable = make_backup_portable(backup["backupId"], str(codex_home_path), passphrase)
    portable_manifest_path = Path(portable["portableManifestPath"])
    portable_manifest = json.loads(portable_manifest_path.read_text(encoding="utf-8"))
    restore_manifest = portable_manifest["restoreManifest"]
    restore_manifest["rowBefore"]["rollout_path"] = "portable://codex-home/../outside.jsonl"
    write_portable_backup_manifest(
        portable_manifest_path,
        restore_manifest,
        passphrase,
        portable_manifest["integrity"]["files"],
    )
...
def test_schema2_backup_remains_listable_and_restorable_without_portable_manifest(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    backup = show_thread_in_sidebar(str(codex_home_path), "thread-0")["backup"]
    manifest_path = tmp_path / "backups" / backup["backupId"] / "manifest.json"
    ...
    listed = list_backups(str(codex_home_path))
    restore_backup(backup["backupId"], str(codex_home_path), create_backup=False)

    assert stored_manifest["schemaVersion"] == 2
    assert listed[0]["portable"] is False
    with closing(sqlite3.connect(codex_home_path / "state_5.sqlite")) as connection:
        title = connection.execute("SELECT title FROM threads WHERE id = 'thread-0'").fetchone()[0]
    assert title == backup["rowBefore"]["title"]


def test_portable_backup_api_preserves_preview_token_and_state_fingerprint_contract(tmp_path: Path) -> None:
    source_home = create_test_codex_home(tmp_path / "source-api")
    target_home = create_test_codex_home(tmp_path / "target-api")
    backup = show_thread_in_sidebar(str(source_home), "thread-0")["backup"]
    passphrase = "portable-api-contract-passphrase"
    client = TestClient(server.app)
    source_auth = client.get("/api/auth/token", params={"codex_home": str(source_home)}).json()
    export_preview = client.post(
        f"/api/backups/{backup['backupId']}/portable/preview",
        params={"codex_home": str(source_home)},
        json={"passphrase": passphrase},
    )
    assert export_preview.status_code == 200
    export_ticket = export_preview.json()
    export_response = client.post(
        f"/api/backups/{backup['backupId']}/portable",
        params={"codex_home": str(source_home)},
        headers={source_auth["headerName"]: source_auth["token"]},
        json={
            "passphrase": passphrase,
            "operationPreviewId": export_ticket["operationPreviewId"],
            "inputHash": export_ticket["inputHash"],
        },
    )
    assert export_response.status_code == 200

    target_auth = client.get("/api/auth/token", params={"codex_home": str(target_home)}).json()
    portable_list = client.get("/api/backups", params={"codex_home": str(target_home)})
    assert portable_list.status_code == 200
    assert portable_list.json()["backups"][0]["foreignPortable"] is True
    restore_preview = client.post(
        f"/api/backups/{backup['backupId']}/portable/restore/preview",
        params={"codex_home": str(target_home)},
        json={"sourceCodexHome": str(source_home), "passphrase": passphrase},
    )
    assert restore_preview.status_code == 200
    restore_ticket = restore_preview.json()
    rollout_backup_path = Path(backup["rolloutBackupPath"])
    original_rollout_backup = rollout_backup_path.read_bytes()
    rollout_backup_path.write_bytes(b"changed after preview")
    stale_restore = client.post(
        f"/api/backups/{backup['backupId']}/portable/restore",
        params={"codex_home": str(target_home)},
        headers={target_auth["headerName"]: target_auth["token"]},
        json={
            "sourceCodexHome": str(source_home),
            "passphrase": passphrase,
            "createBackup": False,
            "operationPreviewId": restore_ticket["operationPreviewId"],
            "inputHash": restore_ticket["inputHash"],
        },
    )
    assert stale_restore.status_code == 409
    assert "state changed after preview" in stale_restore.json()["detail"]
    rollout_backup_path.write_bytes(original_rollout_backup)
    ...
    refreshed_ticket = refreshed_preview.json()
    restored = client.post(
        ...
        json={
            ...
            "passphrase": passphrase,
            "createBackup": False,
            "operationPreviewId": refreshed_ticket["operationPreviewId"],
            "inputHash": refreshed_ticket["inputHash"],
        },
    )
    assert restored.status_code == 200
    assert restored.json()["restoredBackupId"] == backup["backupId"]
...
def install_official_thread_tool_registry(
    ...
    *,
    ...
    with closing(sqlite3.connect(codex_home_path / "state_5.sqlite")) as connection, connection:
        ...
        for position, name in enumerate(sorted(required_thread_tool_names)):
            connection.execute(
                "INSERT INTO thread_dynamic_tools VALUES (?, ?, ?, ?, '{}', 0, 'codex_app')",
                (thread_id, position, name, f"{name} description"),
            )


def test_official_thread_tools_preview_and_repair_cover_initial_rollout_metadata(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    install_official_thread_tool_registry(codex_home_path)
    ...
    assert preview["needsRepair"] is True
    assert preview["willRepairInitialSessionMeta"] is True
    assert preview["willNormalizeThreadToolPositions"] is False
    assert preview["initialSessionMeta"]["candidateCount"] == 1
    assert preview["rolloutBackupRequired"] is True
    assert preview["requiresCodexRestart"] is False

    result = repair_official_thread_tools_exposure(str(codex_home_path))

    assert result["changed"] is True
    assert result["restartRequired"] is False
    assert result["initialSessionMetaRepair"]["state"] == "complete"
    assert result["initialSessionMetaRepair"]["completedCount"] == 1
    assert result["after"]["initialSessionMeta"]["candidateCount"] == 0
    ...
    rollout_backup_path = Path(result["initialSessionMetaRepair"]["threads"][0]["backupPath"])
    assert rollout_backup_path.exists()
    assert scan_rollout(rollout_backup_path).source_sha256 == baseline_scan.source_sha256


def test_official_thread_tools_preview_preserves_detailed_fields_over_http(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    install_official_thread_tool_registry(codex_home_path)
    ...
    response = client.post(
        "/api/codex/official-thread-tools/repair/preview",
        params={"codex_home": str(codex_home_path)},
        headers=origin_headers,
    )
    ...
    assert body["initialSessionMeta"]["actionableCandidateCount"] == 1
    assert body["requiresCodexRestart"] is False
    assert body["operationPreviewId"]
    assert body["inputHash"]


def test_official_thread_tools_preview_closes_sqlite_handles(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    install_official_thread_tool_registry(codex_home_path)
    database_path = codex_home_path / "state_5.sqlite"
    moved_path = codex_home_path / "state_5.sqlite.handle-check"

    preview_official_thread_tools_repair(str(codex_home_path))
...
def test_official_thread_tools_scan_failure_is_blocking_not_no_repair(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    ...
    monkeypatch.setattr(
        ...
        lambda _paths: {
            ...
            "error": "injected schema scan failure",
        },
    )
    ...
    assert preview["needsRepair"] is True
    assert preview["repairBlockedByInspectionError"] is True
    assert preview["willRepairInitialSessionMeta"] is False
    assert preview["inspectionErrors"] == ["injected schema scan failure"]
    with pytest.raises(RuntimeError, match="inspection failed"):
        repair_official_thread_tools_exposure(str(codex_home_path))


def test_combined_official_thread_tools_failure_rolls_back_jsonl(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    ...
    original_replace = codex_data_module.replace_file_with_retry

    def fail_config_install(source_path: Path, target_path: Path) -> None:
        if target_path == codex_home_path / "config.toml" and source_path.name.endswith(
            ".official-thread-tools.tmp"
        ):
            raise RuntimeError("injected config install failure")
        original_replace(source_path, target_path)

    monkeypatch.setattr(codex_data_module, "replace_file_with_retry", fail_config_install)

    with pytest.raises(RuntimeError, match="injected config install failure"):
        repair_official_thread_tools_exposure(str(codex_home_path))

    restored_scan = scan_rollout(rollout_path)
    assert restored_scan.source_sha256 == baseline_scan.source_sha256
    assert restored_scan.user_prompt_count == baseline_scan.user_prompt_count
    assert restored_scan.user_prompt_sha256 == baseline_scan.user_prompt_sha256
    assert "[mcp_servers.codex_thread_messenger]" in (
        codex_home_path / "config.toml"
    ).read_text(encoding="utf-8")


def test_session_meta_id_mismatch_is_not_reported_as_shared_rollout_alias(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    install_official_thread_tool_registry(codex_home_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-0.jsonl"
    records = [json.loads(line) for line in rollout_path.read_text(encoding="utf-8").splitlines()]
    records[0]["payload"]["id"] = "unknown-thread-id"
    rollout_path.write_text(
        "".join(json.dumps(record, ensure_ascii=False, separators=(",", ":")) + "\n" for record in records),
        encoding="utf-8",
    )

    initial_scan = inspect_official_thread_tool_initial_session_meta(resolve_codex_paths(str(codex_home_path)))
    ...
    assert initial_scan["blockedReasonCounts"] == {"session_meta_id_mismatch": 1}
    assert "plugins.official_thread_tools_session_meta_id_mismatch" in issue_ids
    assert "plugins.official_thread_tools_shared_rollout_aliases" not in issue_ids
...
def test_official_thread_tools_repair_normalizes_non_contiguous_registry_positions(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    database_path = codex_home_path / "state_5.sqlite"
    tool_names = [
        "automation_update",
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        for position, name in zip((0, 1, 4, 5, 6), tool_names, strict=True):
            connection.execute(
                "INSERT INTO thread_dynamic_tools VALUES (?, ?, ?, ?, '{}', 0, 'codex_app')",
                ("thread-0", position, name, f"{name} description"),
            )

    preview = preview_official_thread_tools_repair(str(codex_home_path))

    assert preview["needsRepair"] is True
    assert preview["willNormalizeThreadToolPositions"] is True
    assert preview["threadToolRegistry"]["threadsWithNonContiguousPositions"] == 1

    result = repair_official_thread_tools_exposure(str(codex_home_path))

    assert result["changed"] is True
    assert result["normalizedThreadToolPositions"]["normalizedThreadCount"] == 1
    with sqlite3.connect(database_path) as connection:
        repaired_rows = connection.execute(
            "SELECT position, name, description, input_schema, namespace FROM thread_dynamic_tools WHERE thread_id = ? ORDER BY position",
            ("thread-0",),
        ).fetchall()
        assert connection.execute("PRAGMA quick_check").fetchone()[0] == "ok"
    assert [row[0] for row in repaired_rows] == [0, 1, 2, 3, 4]
    assert [row[1] for row in repaired_rows] == tool_names
    assert all(row[2] == f"{row[1]} description" for row in repaired_rows)
    assert all(row[3] == "{}" and row[4] == "codex_app" for row in repaired_rows)
    assert inspect_official_thread_tool_registry(resolve_codex_paths(str(codex_home_path)))[
        "threadsWithNonContiguousPositions"
    ] == 0


def test_diagnostics_reports_non_contiguous_official_thread_tool_positions(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    write_minimal_sandbox_runtime(codex_home_path)
    with sqlite3.connect(codex_home_path / "state_5.sqlite") as connection:
        connection.execute(
        ...
        connection.executemany(
            "INSERT INTO thread_dynamic_tools VALUES ('thread-0', ?, ?, '', '{}', 0, 'codex_app')",
            [(0, "automation_update"), (1, "read_thread_terminal"), (4, "send_message_to_thread")],
        )
    ...
    assert "non_contiguous_thread_tool_threads=1" in exposure_check["evidence"]
    assert any(issue["id"] == "plugins.official_thread_tools_shadowed_by_fallback_mcp" for issue in report["issues"])


def test_diagnostics_reports_incomplete_initial_official_thread_tool_metadata(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    write_minimal_sandbox_runtime(codex_home_path)
    install_official_thread_tool_registry(codex_home_path)
    ...
    exposure_check = next(check for check in report["checks"] if check["id"] == "plugins.official_thread_tools_exposure")
    assert exposure_check["status"] == "warning"
    assert "initial_session_meta_threads_missing_official_tools=1" in exposure_check["evidence"]
    issue = next(
        issue
        for issue in report["issues"]
        if issue["id"] == "plugins.official_thread_tools_initial_metadata_drift"
    )
    assert "首个 session_meta" in issue["summary"]
...
def test_diagnostics_allows_installed_but_disabled_apple_plugin(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    ...
    assert not any(issue["id"] == "plugins.macos_plugin_active_on_windows" for issue in report["issues"])
...
def test_diagnostics_reports_unguarded_computer_use_documentation_contract(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    write_minimal_sandbox_runtime(codex_home_path)
    write_minimal_bundled_plugin_cache(codex_home_path)
    skill_path = (
    ...
    skill_path.write_text(
        "Use `await sky.documentation(\"guidance\")` before Computer Use.\n",
        encoding="utf-8",
    )

    report = run_codex_diagnostics(str(codex_home_path), sidebar_limit=2, language="en")
    checks = {check["id"]: check for check in report["checks"]}
    assert checks["plugins.computer_use_documentation_contract"]["status"] == "warning"
    assert any(
        issue["id"] == "plugins.computer_use_documentation_contract_mismatch"
        for issue in report["issues"]
    )

    skill_path.write_text(
        "if (typeof sky.documentation === \"function\") await sky.documentation(\"guidance\");\n"
        "Otherwise read docs/guidance.md, docs/api.md, and docs/confirmations.md.\n",
        encoding="utf-8",
    )
    repaired_report = run_codex_diagnostics(
        str(codex_home_path), sidebar_limit=2, language="en"
    )
    repaired_checks = {check["id"]: check for check in repaired_report["checks"]}
    assert repaired_checks["plugins.computer_use_documentation_contract"]["status"] == "pass"
    assert not any(
        issue["id"] == "plugins.computer_use_documentation_contract_mismatch"
...
""".lstrip(),
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            ...
            "legacyThreadMessengerProcessCount": 0,
            "xcodebuildMcpProcessCount": 0,
            "nodeReplWithDisableSandboxCount": 1,
            "nodeReplProcesses": [
                {
                    "pid": "42",
                    "name": "node_repl.exe",
                    "commandLine": f'"{node_repl_path}" --disable-sandbox',
                }
            ],
            "sampleProcesses": [],
        },
    ...
    assert checks["plugins.node_repl_desktop_privileged_mode"]["status"] == "pass"
    assert "desktop_native_pipe_contract=True" in checks[
...
def test_diagnostics_reports_node_repl_kernel_asset_write_failure(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    rollout_path = codex_home_path / "sessions" / "rollout-thread-2.jsonl"
    with rollout_path.open("a", encoding="utf-8") as handle:
        handle.write(
            ...
            json.dumps(
                {
                    ...
                    "payload": {
                        ...
                        "output": (
                            "Exit code: 0\n"
                            "diagnostics evidence: failed to write kernel assets: "
    ...
    blocker_check = next(check for check in report["checks"] if check["id"] == "plugins.recent_runtime_blocker_messages")
    evidence_text = "\n".join(blocker_check["evidence"])
    assert "node_repl_kernel_asset_write_failed" in evidence_text
    assert evidence_text.count("node_repl_kernel_asset_write_failed") == 1
    issue = next(issue for issue in report["issues"] if issue["id"] == "plugins.node_repl_kernel_asset_write_failed")
    assert issue["severity"] in {"info", "warning"}
    assert "TEMP" in issue["recommendation"]
    assert ".tmp" in issue["recommendation"]
...
def test_diagnostics_reports_node_repl_asset_directory_probe(
    ...
    temporary_asset_path = tmp_path / "node_repl_assets"
    temporary_asset_path.mkdir()
    ...
    (codex_home_path / "config.toml").write_text(
        "\n".join(
            [
                ...
                f'TEMP = "{temporary_asset_path.as_posix()}"',
                f'TMP = "{temporary_asset_path.as_posix()}"',
                f'TMPDIR = "{temporary_asset_path.as_posix()}"',
                "",
            ]
        ),
        encoding="utf-8",
    ...
    assert str(temporary_asset_path) in asset_check["affectedPaths"]
...
def test_product_version_is_consistent_across_package_openapi_capabilities_and_mcp() -> None:
    ...
    assert server.packaged_product_version == expected_version
    assert server.product_version == expected_version
    assert client.get("/openapi.json").json()["info"]["version"] == expected_version
    assert client.get("/api/capabilities").json()["version"] == expected_version
    assert client.get("/mcp").json()["version"] == expected_version
    initialize_payload = mcp_call(client, 1, "initialize")
    assert initialize_payload["result"]["serverInfo"]["version"] == expected_version


def test_sensitive_console_responses_set_security_headers_without_breaking_cors() -> None:
    public_origin = "https://codex-home-manager.simplezion.com"
    client = TestClient(server.app, base_url="http://127.0.0.1:8765")

    console_response = client.get("/")
    response = client.get("/api/capabilities", headers={"Origin": public_origin})
    preflight = client.options(
        "/api/capabilities",
        headers={
            "Origin": public_origin,
            "Access-Control-Request-Method": "GET",
            "Access-Control-Request-Headers": "x-codex-manager-token",
        },
    )
    ...
    for current_response in (response, preflight):
        assert current_response.headers["Access-Control-Allow-Origin"] == public_origin


def test_preview_parameter_http_exceptions_remain_client_errors(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path)
    client = TestClient(server.app, base_url="http://127.0.0.1:8765")

    duplicate_response = client.get(
        "/api/threads/thread-0/action-preview",
        params={"codex_home": str(codex_home_path), "action": "duplicate"},
    )
    slim_response = client.get(
        "/api/threads/thread-0/slim/preview",
        params={
            "codex_home": str(codex_home_path),
            "removeImages": "false",
            "keepLatestCompacted": "false",
        },
    )

    assert duplicate_response.status_code == 400
    assert duplicate_response.json()["detail"] == "targetProjectPath is required for duplicate preview"
    assert slim_response.status_code == 400
    assert slim_response.json()["detail"] == "select at least one slimming scope"
...
def test_mcp_initialize_tools_list_and_read_tool(tmp_path: Path) -> None:
    ...
    assert "codex_preview_portable_backup_export" in tool_names
    assert "codex_export_portable_backup" in tool_names
    assert "codex_preview_portable_backup_restore" in tool_names
    assert "codex_restore_portable_backup" in tool_names
    assert len(tools) >= 32
    api_token = mcp_local_authorization(client, codex_home_path)
...
def test_mcp_portable_backup_export_and_restore_complete_real_round_trip(tmp_path: Path) -> None:
    codex_home_path = create_test_codex_home(tmp_path / "portable-mcp")
    backup = show_thread_in_sidebar(str(codex_home_path), "thread-0")["backup"]
    backup_id = str(backup["backupId"])
    passphrase = "portable-mcp-round-trip-passphrase"
    client = TestClient(server.app)
    api_token = mcp_local_authorization(client, codex_home_path)

    export_preview_payload = mcp_call(
        client,
        11,
        "tools/call",
        {
            "name": "codex_preview_portable_backup_export",
            "arguments": {
                "codexHome": str(codex_home_path),
                "backupId": backup_id,
                "passphrase": passphrase,
    ...
    assert export_preview_payload["result"].get("isError", False) is False, export_preview_payload
    export_preview = export_preview_payload["result"]["structuredContent"]
    assert export_preview["backupId"] == backup_id
    assert export_preview["operationPreviewId"]

    export_payload = mcp_call(
        client,
        12,
        "tools/call",
        {
            "name": "codex_export_portable_backup",
            "arguments": {
                "codexHome": str(codex_home_path),
                "backupId": backup_id,
                "passphrase": passphrase,
                "apiToken": api_token,
                "operationPreviewId": export_preview["operationPreviewId"],
                "inputHash": export_preview["inputHash"],
            },
        },
    )
    exported = export_payload["result"]["structuredContent"]
    assert exported["backupId"] == backup_id
    assert Path(exported["portableManifestPath"]).is_file()

    restore_preview_payload = mcp_call(
        client,
        13,
        "tools/call",
        {
            "name": "codex_preview_portable_backup_restore",
            "arguments": {
                "codexHome": str(codex_home_path),
                "backupId": backup_id,
                "sourceCodexHome": str(codex_home_path),
                "passphrase": passphrase,
                "apiToken": api_token,
            },
        },
    )
    restore_preview = restore_preview_payload["result"]["structuredContent"]
    assert restore_preview["backupId"] == backup_id
    assert restore_preview["operationPreviewId"]

    restore_payload = mcp_call(
        client,
        14,
        "tools/call",
        {
            "name": "codex_restore_portable_backup",
            "arguments": {
                "codexHome": str(codex_home_path),
                "backupId": backup_id,
                "sourceCodexHome": str(codex_home_path),
                "passphrase": passphrase,
                "apiToken": api_token,
                "operationPreviewId": restore_preview["operationPreviewId"],
                "inputHash": restore_preview["inputHash"],
                "createBackup": False,
            },
        },
    )
    restored = restore_payload["result"]["structuredContent"]
    assert restored["restoredBackupId"] == backup_id
    assert restore_payload["result"].get("isError", False) is False
...
def test_loopback_same_origin_reads_are_frictionless_but_cross_origin_reads_stay_protected(tmp_path: Path) -> None:
    ...
    assert local_agent_without_authorization.status_code == 200
    ...
    cross_site_without_origin = local_client.get(
        "/api/resources/read",
        params={"codex_home": str(codex_home_path), "relative_path": "config.toml"},
        headers={"Sec-Fetch-Site": "cross-site"},
    )
    assert cross_site_without_origin.status_code == 401
