...
@pytest.mark.parametrize("process_name", ["cmd.exe", "extension-host.exe"])
def test_offline_process_guard_ignores_active_chrome_native_messaging_host(
    monkeypatch,
    process_name: str,
) -> None:
    class ChromeNativeMessagingProcess:
        pid = 9753
        info = {"pid": 9753, "name": process_name}
        ...
        def cmdline(self):
            return [
                ...
                "--parent-window=0",
                r"\\.\pipe\chrome.nativeMessaging.in.123",
                r"\\.\pipe\chrome.nativeMessaging.out.123",
            ]

        def exe(self):
            return r"C:\Windows\System32\cmd.exe" if process_name == "cmd.exe" else self.cmdline()[0]

    monkeypatch.setattr(
        offline_repair_policy.psutil,
        "process_iter",
        lambda _attributes: [ChromeNativeMessagingProcess()],
    )
