...
type BrowserPromptJsonContext =
  | { kind: "object"; path: string[]; key: string | null; expectingKey: boolean }
  | { kind: "array"; path: string[] };

const browserJsonlChunkBytes = 512 * 1024;
const browserJsonlMaxRecordBytes = 8 * 1024 * 1024;
const browserThreadHeadSampleBytes = 2 * 1024 * 1024;
...
class BrowserBoundedJsonString {
  value = "";
  truncated = false;
  ...
  private appendVisible(character: string): void {
...
class BrowserOversizedPromptScanner {
  private readonly decoder = new TextDecoder("utf-8");
  private readonly stack: BrowserPromptJsonContext[] = [];
  private inString = false;
  private stringIsKey = false;
  private stringPath: string[] = [];
  private stringValue: BrowserBoundedJsonString | null = null;
  private escaped = false;
  private unicodeEscape: string | null = null;
  private extractedCharacters = 0;
  private textTruncated = false;
  private topLevelType = "";
  private payloadType = "";
  private payloadRole = "";
  private timestamp: string | null = null;
  private readonly payloadMessageParts: string[] = [];
  private readonly payloadTextParts: string[] = [];
  private readonly payloadContentParts: string[] = [];

  feed(bytes: Uint8Array): void {
    this.feedText(this.decoder.decode(bytes, { stream: true }));
  }

  finish(): BrowserOversizedPromptCandidate | null {
    this.feedText(this.decoder.decode());
    const protocol = this.protocol();
    if (!protocol) return null;
    const message = this.payloadMessageParts.filter(Boolean).join("\n");
    const textValue = this.payloadTextParts.filter(Boolean).join("\n");
    const content = this.payloadContentParts.filter(Boolean).join("\n");
    const text = protocol === "response_item"
      ? content
      : message || textValue || content;
    const trimmedText = text.trim();
    if (!trimmedText) return null;
    return {
      timestamp: this.timestamp,
      text: this.textTruncated ? `${trimmedText}${browserPromptTruncationNotice}` : trimmedText,
      protocol,
  ...
  private protocol(): string {
    if (this.topLevelType === "user_message") return "user_message";
    if (this.topLevelType === "event_msg" && this.payloadType === "user_message") return "event_msg";
    if (this.topLevelType === "response_item" && this.payloadType === "message" && this.payloadRole === "user") {
  ...
  private consumeCharacter(character: string): void {
    ...
    if (character === "\"") {
      ...
      if (context?.kind === "object") context.expectingKey = false;
    } else if (character === ",") {
      const context = this.stack.at(-1);
  ...
  private consumeStringCharacter(character: string): void {
    if (this.unicodeEscape !== null) {
      ...
      if (this.unicodeEscape.length === 4) {
        const codeUnit = Number.parseInt(this.unicodeEscape, 16);
        this.stringValue?.append(Number.isFinite(codeUnit) ? String.fromCharCode(codeUnit) : "");
    ...
    if (this.escaped) {
      ...
      this.stringValue?.append(escapedCharacter[character] ?? character);
      return;
    }
    if (character === "\\") {
      this.escaped = true;
    ...
    this.stringValue?.append(character);
  }
  ...
  private startString(): void {
    ...
    this.escaped = false;
    this.unicodeEscape = null;
    this.stringIsKey = context?.kind === "object" && context.expectingKey;
    this.stringPath = this.stringIsKey ? [] : this.nextValuePath();
    const collectPromptText = !this.stringIsKey && this.shouldCollectValue(this.stringPath);
    const collectMetadata = !this.stringIsKey && this.shouldCollectMetadata(this.stringPath);
    const characterLimit = this.stringIsKey
      ? 256
      : collectPromptText
        ? Math.max(0, browserPromptMaxExtractedCharacters - this.extractedCharacters)
        : collectMetadata
          ? 1_024
          : 0;
    this.stringValue = this.stringIsKey || collectPromptText || collectMetadata
      ? new BrowserBoundedJsonString(characterLimit, !this.stringIsKey && this.isPromptTextPath(this.stringPath))
      : null;
  }
  ...
  private finishString(): void {
    ...
    if (this.stringIsKey) {
      const context = this.stack.at(-1);
      if (context?.kind === "object") context.key = value;
    } else {
      this.acceptValue(this.stringPath, value, Boolean(this.stringValue?.truncated));
    }
    this.inString = false;
  ...
  private startContainer(kind: "object" | "array"): void {
    const path = this.stack.length ? this.nextValuePath() : [];
    this.stack.push(kind === "object"
      ? { kind, path, key: null, expectingKey: true }
      : { kind, path });
  }

  private nextValuePath(): string[] {
    const context = this.stack.at(-1);
    if (!context) return [];
  ...
  private shouldCollectMetadata(path: string[]): boolean {
    const joined = path.join(".");
    return joined === "type" || joined === "timestamp" || joined === "payload.type" || joined === "payload.role";
  }

  private shouldCollectValue(path: string[]): boolean {
    return this.isPromptTextPath(path);
  }

  private isPromptTextPath(path: string[]): boolean {
    if (path.length === 1 && path[0] === "payload") return true;
    if (path[0] !== "payload" || !["message", "text", "content"].includes(path[1])) return false;
    if (path.length === 2) return true;
    const last = path.at(-1) || "";
    return last === "*" || ["text", "message", "content", "value"].includes(last);
  }

  private acceptValue(path: string[], value: string, truncated: boolean): void {
    const joined = path.join(".");
    if (joined === "type") this.topLevelType = value;
    else if (joined === "timestamp") this.timestamp = value || null;
    else if (joined === "payload.type") this.payloadType = value;
    else if (joined === "payload.role") this.payloadRole = value;

    if (!this.isPromptTextPath(path)) return;
    this.extractedCharacters += value.length;
    this.textTruncated ||= truncated;
    if (path.length === 1) this.payloadContentParts.push(value);
    else if (path[1] === "message") this.payloadMessageParts.push(value);
    else if (path[1] === "text") this.payloadTextParts.push(value);
    else this.payloadContentParts.push(value);
  }
}

function decodeBrowserJsonlParts(parts: Uint8Array[], totalBytes: number): string {
  ...
  const decoded = browserJsonlDecoder.decode(combined);
...
async function* readBrowserJsonlLines(
  ...
  let lineBytes = 0;
  let oversized = false;
  let oversizedPromptScanner: BrowserOversizedPromptScanner | null = null;

  const appendPart = (part: Uint8Array) => {
    if (!part.byteLength) return;
    if (oversized) {
      oversizedPromptScanner?.feed(part);
      return;
    }
    if (lineBytes + part.byteLength > browserJsonlMaxRecordBytes) {
      if (options.extractOversizedPrompts) {
        oversizedPromptScanner = new BrowserOversizedPromptScanner();
        for (const bufferedPart of lineParts) oversizedPromptScanner.feed(bufferedPart);
        oversizedPromptScanner.feed(part);
      }
      lineParts = [];
      lineBytes = 0;
  ...
  while (position < stopAfterByte) {
    options.signal?.throwIfAborted();
    const blockEnd = Math.min(stopAfterByte, position + browserJsonlChunkBytes);
    const bytes = new Uint8Array(await file.slice(position, blockEnd).arrayBuffer());
    let segmentStart = 0;
    for (let index = 0; index < bytes.byteLength; index += 1) {
      if (bytes[index] !== 10) continue;
      appendPart(bytes.subarray(segmentStart, index));
      lineNumber += 1;
      yield {
        lineNumber,
      ...
      lineBytes = 0;
      oversized = false;
      oversizedPromptScanner = null;
      lineByteOffset = position + index + 1;
  ...
  if (lineByteOffset < stopAfterByte && stopAfterByte === file.size) {
    lineNumber += 1;
    yield {
      lineNumber,
      byteOffset: lineByteOffset,
...
async function* readBrowserJsonlLinesReverse(
  ...
  const prependPart = (part: Uint8Array) => {
    if (!part.byteLength || oversized) return;
    if (suffixBytes + part.byteLength > browserJsonlMaxRecordBytes) {
      suffixParts = [];
      suffixBytes = 0;
      oversized = true;
      return;
    ...
    suffixBytes += part.byteLength;
  };

  while (position > stopBeforeByte) {
    options.signal?.throwIfAborted();
    const blockStart = Math.max(stopBeforeByte, position - browserJsonlChunkBytes);
    const bytes = new Uint8Array(await file.slice(blockStart, position).arrayBuffer());
    let segmentEnd = bytes.byteLength;
    for (let index = bytes.byteLength - 1; index >= 0; index -= 1) {
      if (bytes[index] !== 10) continue;
      prependPart(bytes.subarray(index + 1, segmentEnd));
      yield {
        byteOffset: blockStart + index + 1,
        rawLine: oversized ? null : decodeBrowserJsonlParts(suffixParts, suffixBytes),
        oversized,
        oversizedPrompt: null
      };
      suffixParts = [];
      suffixBytes = 0;
      oversized = false;
    ...
    prependPart(bytes.subarray(0, segmentEnd));
    position = blockStart;
  }

  if ((suffixBytes || oversized) && stopBeforeByte === 0) {
    yield {
      byteOffset: stopBeforeByte,
      rawLine: oversized ? null : decodeBrowserJsonlParts(suffixParts, suffixBytes),
      oversized,
      oversizedPrompt: null
    };
...
export async function readBrowserThreadDetail(workspace: BrowserCodexWorkspace, threadId: string): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    rolloutStats: {
      ...
      toolOutputs: 0,
      eventMessages: 0,
      invalidJsonLines: sample.aggregationExact ? sample.parseErrors : null,
      aggregationExact: sample.aggregationExact,
      oversizedRecords: sample.oversizedRecords,
      uncertaintyReasons: sample.aggregationExact ? [] : ["oversized_jsonl_record"],
...
async function buildBrowserDailyTokenUsage(workspace: BrowserCodexWorkspace, threadId: string) {
  ...
  const allUsages = [ownUsage, ...childUsages];
  const aggregationExact = allUsages.every((usage) => usage.summary.aggregationExact !== false);
  const oversizedRecords = allUsages.reduce((total, usage) => total + Number(usage.summary.oversizedRecords || 0), 0);
  ...
  return {
    summary: {
      ...
      aggregationExact,
      oversizedRecords,
      uncertaintyReasons: aggregationExact ? [] : ["oversized_jsonl_record"]
    },
    days
  };
}
...
async function readBrowserThreadFileDailyTokenUsage(threadFile: BrowserThreadFile | undefined, thread?: Record<string, unknown>) {
  ...
  const file = await threadFile.handle.getFile();
  const usage = await parseBrowserDailyTokenUsage(file);
...
async function parseBrowserDailyTokenUsage(file: Pick<File, "size" | "slice">) {
  ...
  for await (const line of readBrowserJsonlLines(file)) {
    ...
    if (line.oversized || line.rawLine === null) {
      result.summary.aggregationExact = false;
      result.summary.oversizedRecords += 1;
      continue;
    }
  ...
  if (!result.summary.aggregationExact) result.summary.uncertaintyReasons = ["oversized_jsonl_record"];
...
export async function readBrowserThreadLogs(
  ...
  for await (const jsonlLine of readBrowserJsonlLines(file)) {
    const line = jsonlLine.rawLine;
    if (line === "") continue;
    if (jsonlLine.oversized || line === null) {
      oversizedRecords += 1;
      continue;
    }
    ...
    const entry = {
      ...
      lineNumber: jsonlLine.lineNumber,
      appLogId: null,
    ...
    if (!kindMatches || !searchMatches) continue;
    byKind[entry.kind] = (byKind[entry.kind] || 0) + 1;
    bySeverity[entry.severity] = (bySeverity[entry.severity] || 0) + 1;
    if (matchedEntries >= safeOffset && entries.length < safeLimit) entries.push(entry);
    matchedEntries += 1;
  }
  const aggregationExact = oversizedRecords === 0;
  return {
    threadId,
    source: "rollout",
    rolloutPath: threadFile.relativePath,
    ...
    search,
    matchedEntries,
    matchedEntriesExact: aggregationExact,
    matchedEntriesLowerBound: matchedEntries,
    hasMore: safeOffset + entries.length < matchedEntries,
    entries,
    aggregationExact,
    oversizedRecords,
    uncertaintyReasons: aggregationExact ? [] : ["oversized_jsonl_record"],
    summary: {
      lineCount: matchedEntries,
      parseErrors,
      byKind,
      bySeverity,
      aggregationExact,
...
export async function readBrowserThreadTimeline(
  workspace: BrowserCodexWorkspace,
  threadId: string,
  ...
  const file = await threadFile.handle.getFile();
  const thread = (workspace.snapshot.threads as Record<string, unknown>[] | undefined)?.find((item) => item.id === threadId);
  return readBrowserTimelinePage(file, threadId, String(thread?.title || ""), threadFile.relativePath, options);
}

export async function readBrowserThreadTimelineItem(
  workspace: BrowserCodexWorkspace,
  threadId: string,
  byteOffset: number
  ...
  const threadFile = workspace.threadFiles.get(threadId);
  if (!threadFile) throw new Error(`Thread not found in selected folder: ${threadId}`);
  return readBrowserTimelineItem(await threadFile.handle.getFile(), byteOffset);
}
...
async function readSessionIndexRecords(root: BrowserDirectoryHandle): Promise<BrowserSessionIndexRecord[]> {
  ...
  for await (const line of readBrowserJsonlLines(file)) {
    ...
    records.push({
      ...
      sessionIndexLine: line.lineNumber
    });
  }
  return records;
}
...
async function readBrowserRolloutThreadTitleUpdate(threadFile: BrowserThreadFile, threadId: string) {
  ...
  for await (const line of readBrowserJsonlLines(file)) {
    ...
    result.rolloutTitleLine = line.lineNumber;
  }
  return result;
}
...
async function parseThreadSample(file: File, full = false): Promise<ParsedThreadSample> {
  ...
  const state = {
    ...
    cliVersion: "",
    gitBranch: "",
    threadKind: "main" as "main" | "subagent",
    threadSource: "",
    parentThreadId: "",
    subagentStatus: "",
    agentNickname: "",
  ...
  const consumeLine = (rawLine: string | null, oversized: boolean) => {
    if (rawLine === "") return;
    state.lineCount += 1;
    if (oversized || rawLine === null) {
      state.oversizedRecords += 1;
      return;
    }
    const parsed = parseJsonLine(rawLine);
    if (!parsed) {
      state.parseErrors += 1;
      rolloutDisplay.parseErrors += 1;
      return;
    }
    addRolloutDisplayItem(rolloutDisplay, parsed);
    const timestampMs = timestampMsFromObject(parsed);
    if (timestampMs) {
      state.createdAtMs = state.createdAtMs ? Math.min(state.createdAtMs, timestampMs) : timestampMs;
      state.updatedAtMs = state.updatedAtMs ? Math.max(state.updatedAtMs, timestampMs) : timestampMs;
    }
    state.projectPath ||= firstStringByKeys(parsed, ["cwd", "current_working_directory", "projectPath", "project_path"]) || "";
    state.model ||= firstStringByKeys(parsed, ["model", "model_slug"]) || "";
    state.cliVersion ||= firstStringByKeys(parsed, ["cli_version", "version", "codex_version"]) || "";
    state.gitBranch ||= firstStringByKeys(parsed, ["git_branch", "branch"]) || "";
    const metadata = extractThreadMetadata(parsed, rawLine);
    if (metadata.threadKind === "subagent") state.threadKind = "subagent";
    state.threadSource ||= metadata.threadSource;
    state.parentThreadId ||= metadata.parentThreadId;
    state.subagentStatus ||= metadata.subagentStatus;
    state.agentNickname ||= metadata.agentNickname;
    state.agentRole ||= metadata.agentRole;
    state.tokensUsed += firstNumberByKeys(parsed, ["total_tokens", "tokens_used", "tokensUsed"]) || 0;
    ...
    if (!state.hasUserEvent && (role === "user" || type.includes("user"))) {
      state.hasUserEvent = true;
      state.title ||= compactText(message).slice(0, 80);
      state.preview ||= compactText(message).slice(0, 180);
    }
  };
  ...
  if (exactScan) {
    ...
    for (let index = tailLines.length - 1; index >= 0; index -= 1) {
      consumeLine(tailLines[index].rawLine, tailLines[index].oversized);
    }
  }

  if (state.threadKind !== "subagent" && state.title.toLowerCase().startsWith("# agents.md instructions for")) {
    state.threadKind = "subagent";
    state.threadSource ||= "subagent_title_heuristic";
  }
  rolloutDisplay.status = exactScan && state.oversizedRecords === 0
    ? rolloutDisplayStatusFromCounts(rolloutDisplay)
    : "not_scanned";
  return {
    ...state,
    rolloutDisplayStatus: rolloutDisplay.status,
    ...
    rolloutDisplayEventUserMessages: rolloutDisplay.eventUserMessages,
    rolloutDisplayEventAgentMessages: rolloutDisplay.eventAgentMessages,
    aggregationExact: exactScan && state.oversizedRecords === 0
  };
}
...
function addRolloutDisplayItem(display: RolloutDisplayIntegrity, parsed: any): void {
  const itemType = String(parsed.type || "");
  const payload = parsed.payload && typeof parsed.payload === "object" ? parsed.payload : {};
  if (itemType === "user_message") {
    const message = stringifyContent(payload);
    if (isRealUserPromptText(message)) display.visibleUserMessages += 1;
  } else if (itemType === "event_msg") {
    const payloadType = String(payload.type || "");
    const message = eventPayloadMessageTextBrowser(payload);
    if (payloadType === "user_message" && isRealUserPromptText(message)) {
    ...
    if (isRealUserPromptText(responseRoleMessageTextBrowser(payload, "user"))) display.responseUserMessages += 1;
    if (responseRoleMessageTextBrowser(payload, "assistant")) display.responseAssistantMessages += 1;
  }
}
...
async function readBrowserRolloutDisplayIntegrity(threadFile: BrowserThreadFile | undefined): Promise<RolloutDisplayIntegrity> {
  ...
  for await (const line of readBrowserJsonlLines(file)) {
    ...
    addRolloutDisplayItem(display, parsed);
  }
  display.status = oversizedRecords ? "not_scanned" : rolloutDisplayStatusFromCounts(display);
  return display;
}
...
function browserPromptTextFromItem(value: any): BrowserOversizedPromptCandidate | null {
  ...
  let protocol = "";
  if (itemType === "user_message") {
    text = typeof rawPayload === "string" ? rawPayload : extractMessage({ payload: rawPayload });
    protocol = "user_message";
  } else if (itemType === "event_msg" && String(payload.type || "") === "user_message") {
    text = stringifyContent(payload.message || payload.text || payload.content);
    protocol = "event_msg";
  } else if (
    ...
    text = stringifyContent(payload.content);
    protocol = "response_item";
  }
  const trimmedText = hideBrowserPromptAttachmentData(text).trim();
  if (!protocol || !trimmedText) return null;
  return {
    timestamp: timestampFromObject(value),
    text: trimmedText,
    protocol,
    textTruncated: false
  };
...
async function extractUserPromptRecords(file: Pick<File, "size" | "slice">): Promise<{
  ...
  for await (const line of readBrowserJsonlLines(file, { extractOversizedPrompts: true })) {
    ...
    if (line.oversized || line.rawLine === null) {
      oversizedRecords += 1;
      candidate = line.oversizedPrompt;
    } else {
      const parsed = parseJsonLine(line.rawLine);
      if (parsed) candidate = browserPromptTextFromItem(parsed);
    }
