...
export function capabilityResponseIsCompatible(
  value: unknown,
  frontendVersion = String(packageMetadata.version)
): value is CapabilityResponse {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const response = value as Record<string, unknown>;
  const connectorMajor = typeof response.version === "string" ? semanticVersionMajor(response.version) : null;
  ...
  return response.service === "codex-home-manager"
    ...
    && (response.language === "zh" || response.language === "en")
    && response.openapiPath === "/openapi.json"
    && response.mcpPath === "/mcp"
    && objectField("safetyModel")
    && objectField("commonQueryParameters")
    && Array.isArray(response.capabilities);
}

export function connectorReleaseTrustMessage(
  metadata: ConnectorReleaseMetadata | null | undefined,
  language: Language
): string {
  if (metadata === undefined) {
    return language === "en"
      ? "Checking the published EXE signature metadata. Do not infer Windows code-signing trust until this check finishes."
      : "正在读取已发布 EXE 的签名元数据；完成前不能推断 Windows 代码签名信任状态。";
  }
  if (metadata === null) {
    return language === "en"
      ? "The published EXE signature metadata is unavailable, so its Windows code-signing trust is not confirmed here. Authenticity can still be checked with the detached Ed25519 release manifest and pinned public-key fingerprint."
      : "当前无法读取已发布 EXE 的签名元数据，因此这里不确认其 Windows 代码签名信任状态；仍可通过 detached Ed25519 发布清单和固定公钥指纹核验真实性。";
  }
  ...
  return language === "en"
    ...
    : `发布元数据未记录有效的 Authenticode 结果${status ? `（状态：${status}）` : ""}；这不能作为文件完全未签名的证据。Windows 可能提示风险，真实性应通过 detached Ed25519 发布清单和固定公钥指纹核验。`;
}
...
type LocalApiConnectionStatus = "checking" | "launching" | "connected" | "blocked";
...
const connectorKnownInstalledStorageKey = "codex-home-manager-connector-known-installed";
const returningProductUserAtPageLoad = (() => {
  try {
    return window.localStorage.getItem(languageStorageKey) !== null;
  } catch {
    return false;
  }
})();
...
const localConnectorVerifierDownloadUrl = `${hostedReleaseAssetBaseUrl}/codex-home-manager-verifier-win-x64.exe`;
...
const localConnectorStartupTimeoutMs = 15_000;
const localConnectorStartupPollIntervalMs = 500;
...
function connectorIsKnownInstalled(): boolean {
  try {
    return window.localStorage.getItem(connectorKnownInstalledStorageKey) === "1";
  } catch {
    return false;
  }
}

function rememberInstalledConnector(): void {
  try {
    window.localStorage.setItem(connectorKnownInstalledStorageKey, "1");
  } catch {
    // The connector still launches when storage is unavailable; only automatic launch is not remembered.
  }
}
...
export async function probeLocalConnector(): Promise<boolean> {
  const probeUrl = `${defaultLocalApiBaseUrl}/api/capabilities`;
  const controller = new AbortController();
  const timeoutId = window.setTimeout(() => controller.abort(), 1_500);
  try {
    const response = await fetch(probeUrl, apiRequestOptions(probeUrl, {
      cache: "no-store",
      signal: controller.signal
    }));
    if (!response.ok) return false;
    return capabilityResponseIsCompatible(await response.json());
  } catch {
    return false;
  } finally {
...
async function waitForLocalConnector(timeoutMs = localConnectorStartupTimeoutMs): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  do {
    if (await probeLocalConnector()) return true;
    await delay(localConnectorStartupPollIntervalMs);
  } while (Date.now() < deadline);
  return false;
}

function openLocalConsole(): void {
  rememberInstalledConnector();
  window.location.replace(`${defaultLocalApiBaseUrl}/`);
}

function requestInstalledConnectorLaunch(): void {
  window.location.assign(localConnectorLaunchUrl);
}
...
function localApiConnectionTitle(language: Language, status: LocalApiConnectionStatus): string {
  if (language === "en") {
    if (status === "checking") return "Connecting to the local scanner";
    if (status === "launching") return "Starting the local connector";
    return "Start local scanning";
  }
  if (status === "checking") return "正在连接本机扫描器";
  if (status === "launching") return "正在启动本机连接器";
  return "启动本机扫描";
}
...
function localApiConnectionDescription(language: Language, status: LocalApiConnectionStatus): string {
  if (status === "launching") {
    return language === "en"
      ? "The installed Windows connector is being opened. This page will switch to the local full mode when it is ready."
      : "正在打开已安装的 Windows 连接器；连接就绪后会自动切换到本机完整模式。";
  }
  if (status === "checking") {
    return language === "en"
...
const englishText: Record<string, string> = {
  ...
  "搜索当前筛选的全部内容": "Search all content in the current filter",
  ...
  "查看线程内容": "View thread content",
  "线程内容": "Thread content",
  "线程内容视图": "Thread content views",
  "完整时间线": "Full timeline",
  "我的输入": "My input",
  "主要内容": "Main content",
  "思考过程": "Progress updates",
  "最终回复": "Final replies",
  "推理记录": "Reasoning records",
  "回复": "Replies",
  "思考": "Reasoning",
  "时间线类型筛选": "Timeline type filters",
  "搜索回复、进度说明、推理记录或工具内容": "Search replies, progress updates, reasoning records, or tool content",
  "复制已加载": "Copy loaded",
  "刷新时间线": "Refresh timeline",
  "条已加载": "loaded",
  "本页扫描": "Scanned",
  "条原始记录": "raw records",
  "读取线程时间线...": "Loading thread timeline...",
  "读取更早内容": "Load older content",
  "正在读取更早内容...": "Loading older content...",
  "正在读取完整内容...": "Loading full content...",
  "当前筛选没有可显示的线程内容": "No thread content matches the current filter",
  "该条推理只存储了加密内容，无法读取正文。": "This reasoning record contains only encrypted content; its text is not readable.",
  "该记录另含加密推理正文；这里只显示可读摘要。": "This record also contains encrypted reasoning; only its readable summary is shown.",
  "内容过长，当前副本仅包含已读取部分。展开该条记录可继续读取。": "Content is long; this copy contains only the loaded portion. Expand the entry to load more.",
  "该记录超过单次读取上限，已显示可安全读取的部分。": "This record exceeds the single-read limit; the safely readable portion is shown.",
  "当前为内容预览；折叠后重新展开可重试完整读取。": "This is a content preview. Collapse and expand again to retry loading the full record.",
  ...
  "Prompt 内容列表": "Prompt content list",
...
function normalizePromptRecord(prompt: PromptRecord): PromptRecord {
  ...
  return {
    ...
    sourceType: correctStaleRuntimeClassification ? classification.sourceType : prompt.sourceType || classification.sourceType,
    sourceLabel: correctStaleRuntimeClassification ? classification.sourceLabel : prompt.sourceLabel || classification.sourceLabel,
    visibleByDefault: correctStaleRuntimeClassification
      ? classification.visibleByDefault
      : typeof prompt.visibleByDefault === "boolean" ? prompt.visibleByDefault : classification.visibleByDefault,
    pureText: correctStaleRuntimeClassification
      ? classification.pureText
      : typeof prompt.pureText === "string" ? prompt.pureText : classification.pureText,
    pureCharacterCount: correctStaleRuntimeClassification
      ? classification.pureCharacterCount
      : typeof prompt.pureCharacterCount === "number" ? prompt.pureCharacterCount : classification.pureCharacterCount,
    hasPureText: correctStaleRuntimeClassification
      ? classification.hasPureText
      : typeof prompt.hasPureText === "boolean" ? prompt.hasPureText : classification.hasPureText
  };
}
...
async function fetchThreadTimelineFromLocalApi(
  thread: ThreadRecord,
  codexHome: string,
  kind: TimelineFilter,
  search: string,
  beforeByte: number | null = null,
  signal?: AbortSignal
): Promise<ThreadTimeline> {
  const params = new URLSearchParams({ codex_home: codexHome, kind, search, limit: "80", content_limit: "120000" });
  if (beforeByte !== null) params.set("before_byte", String(beforeByte));
  return fetchJson<ThreadTimeline>(`/api/threads/${thread.id}/timeline?${params.toString()}`, { signal });
}

async function fetchThreadTimelineItemFromLocalApi(
  thread: ThreadRecord,
  codexHome: string,
  byteOffset: number
...
function useSnapshot(codexHome: string, sidebarLimit: number, enabled: boolean) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      const summary = await fetchJson<Omit<Snapshot, "threads">>(`/api/snapshot/summary?${params.toString()}`);
      setSnapshot({ ...summary, threads: [] });
    } catch (refreshError) {
...
function ThreadVisualSummary({ snapshot }: { snapshot: Snapshot }) {
  const { t } = useI18n();
  const statusCount = (status: string) => snapshot.statusCounts?.[status]
    ?? snapshot.threads.filter((thread) => thread.visibility === status).length;
  const baseStatusRows: StatusVisualRow[] = [
    { key: "visible", label: t("可见"), value: statusCount("visible") + statusCount("hidden_by_initial_limit"), tone: "green" },
    { key: "hidden", label: t("隐藏"), value: statusCount("hidden"), tone: "red" },
    { key: "repair", label: t("需修复"), value: statusCount("needs_user_event_repair") + statusCount("missing_file"), tone: "red" },
    { key: "archived", label: t("已归档"), value: statusCount("archived"), tone: "neutral" },
    { key: "subagent", label: t("子agent"), value: statusCount("subagent"), tone: "blue" }
  ];
  ...
  const largestThreads = [...(snapshot.threadHighlights || snapshot.threads)]
...
function ThreadTable({
  ...
  return (
    ...
    >
      ...
      {hasMore ? (
        <div className="thread-page-footer">
  ...
  return renderPanel(
    <>
      ...
      <div className="detail-actions">
        ...
        <button onClick={() => onViewPrompts(thread)}><BookOpen size={15} /> {t("查看线程内容")}</button>
...
function ThreadsModule({
  ...
  const serverPaged = Boolean(snapshot && !snapshot.codexHome.startsWith("browser://"));
  const [debouncedSearchText, setDebouncedSearchText] = React.useState("");
  const [pagedThreads, setPagedThreads] = React.useState<ThreadRecord[]>([]);
  const [threadPage, setThreadPage] = React.useState<ThreadPage | null>(null);
  const [threadPageLoading, setThreadPageLoading] = React.useState(false);
  const [threadPageError, setThreadPageError] = React.useState("");
  ...
  const pageQueryString = React.useMemo(() => new URLSearchParams({
    codex_home: snapshot?.codexHome || "",
    sidebar_limit: String(snapshot?.sidebarLimit || 50),
    thread_kind: threadKindFilter,
    ...
    limit: "50"
  }).toString(), [debouncedSearchText, filterMode, selectedProject, snapshot?.codexHome, snapshot?.sidebarLimit, sortDirection, sortMode, threadKindFilter]);
  ...
  React.useEffect(() => {
    ...
    void fetchJson<ThreadPage>(`/api/threads/page?${pageQueryString}`, { cache: "no-store", signal: controller.signal })
      ...
      .catch((error) => {
        if (pageRequestSequenceRef.current !== requestSequence || (error instanceof DOMException && error.name === "AbortError")) return;
        setThreadPageError(error instanceof Error ? error.message : String(error));
      })
      .finally(() => {
        if (pageRequestSequenceRef.current === requestSequence) setThreadPageLoading(false);
      });
    return () => controller.abort();
  }, [pageQueryString, pageResetNonce, serverPaged, snapshot?.catalogVersion]);

  const loadMoreThreads = React.useCallback(() => {
    if (!serverPaged || !threadPage?.hasMore || !threadPage.nextCursor || threadPageLoading) return;
    const requestSequence = pageRequestSequenceRef.current;
    const controller = new AbortController();
    pageAbortControllerRef.current = controller;
    setThreadPageLoading(true);
    ...
    params.set("cursor", threadPage.nextCursor);
    void fetchJson<ThreadPage>(`/api/threads/page?${params.toString()}`, { cache: "no-store", signal: controller.signal })
      .then((page) => {
        if (pageRequestSequenceRef.current !== requestSequence) return;
        setPagedThreads((current) => {
          const knownIds = new Set(current.map((thread) => thread.id));
          return [...current, ...page.threads.filter((thread) => !knownIds.has(thread.id))];
        });
        setThreadPage(page);
      })
      .catch((error) => {
        if (pageRequestSequenceRef.current !== requestSequence || (error instanceof DOMException && error.name === "AbortError")) return;
        setThreadPageError(error instanceof Error ? error.message : String(error));
        setPageResetNonce((value) => value + 1);
      })
      .finally(() => {
        if (pageRequestSequenceRef.current === requestSequence) setThreadPageLoading(false);
      });
  }, [pageQueryString, serverPaged, threadPage, threadPageLoading]);
  ...
  const filteredThreads = React.useMemo(() => {
    if (!snapshot) return [];
    if (serverPaged) return pagedThreads;
  ...
  }, [filterMode, language, pagedThreads, searchText, selectedProject, serverPaged, snapshot, sortDirection, sortMode, threadKindFilter]);

  const projectCounts = React.useMemo(() => {
    if (!snapshot) return {};
    if (serverPaged) return threadPage?.projectCounts || {};
  ...
  }, [filterMode, serverPaged, snapshot, threadKindFilter, threadPage?.projectCounts]);

  React.useEffect(() => {
    if (!snapshot || !selectedThreadId || (serverPaged && threadPageLoading)) return;
    if (!filteredThreads.some((thread) => thread.id === selectedThreadId)) {
      onClearSelection();
      setIsInspectorOpen(false);
    }
  }, [filteredThreads, onClearSelection, selectedThreadId, serverPaged, snapshot, threadPageLoading]);

  React.useEffect(() => {
    if (!snapshot || !fullDetailThreadId || (serverPaged && threadPageLoading)) return;
    if (!filteredThreads.some((thread) => thread.id === fullDetailThreadId)) {
      setFullDetailThreadId(null);
    }
  }, [filteredThreads, fullDetailThreadId, serverPaged, snapshot, threadPageLoading]);
  ...
  const readOnlyMode = snapshot.codexHome.startsWith("browser://") || filteredThreads.some((thread) => thread.source === "browser-folder");
  const fullDetailThread = fullDetailThreadId ? filteredThreads.find((thread) => thread.id === fullDetailThreadId) || null : null;
      ...
      <div className={`workspace ${isInspectorOpen ? "inspector-open" : "inspector-collapsed"}`}>
        ...
        <section className="main-panel">
          ...
          <div className="result-strip" role="status" aria-live="polite">
            <span>{t("当前结果")} {formatCount(serverPaged ? (threadPage?.matchedThreads || 0) : filteredThreads.length)} {t("条")}</span>
            {serverPaged ? <span>{t("已加载")} {formatCount(filteredThreads.length)}</span> : null}
            ...
            {threadPageError ? <span className="error-text">{threadPageError}</span> : null}
            <button
        ...
        <div className="thread-detail-body">
          ...
          <details className="thread-detail-card full-detail-operations">
            ...
            <div className="full-detail-action-grid">
              ...
              <button onClick={() => onViewPrompts(thread)} type="button"><BookOpen size={15} /> {t("查看线程内容")}</button>
...
function ApiModule({ capabilities }: { capabilities: CapabilityResponse | null }) {
  ...
  const endpointUrl = (endpoint: string) => /^https?:\/\//i.test(endpoint)
    ? endpoint
    : `${baseUrl.replace(/\/$/, "")}/${endpoint.replace(/^\//, "")}`;
  const openapiUrl = endpointUrl(openapiPath);
  const mcpUrl = endpointUrl(mcpPath);
...
function ThreadTimelinePanel({
  ...
  const [timeline, setTimeline] = React.useState<ThreadTimeline | null>(null);
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: (index) => expanded.has(items[index]?.id) ? 220 : 76,
    overscan: 8,
  ...
  async function loadOlder(): Promise<number> {
    ...
    await loadPage(timeline.nextBeforeByte);
  ...
  async function expandItem(item: TimelineItem) {
    ...
    if (!item.textTruncated || fullItems[item.id] || loadingFullItems.has(item.id)) return;
    setLoadingFullItems((current) => new Set(current).add(item.id));
    try {
      const fullItem = browserWorkspace
        ? await readBrowserThreadTimelineItem(browserWorkspace, thread.id, item.byteOffset)
        : await fetchThreadTimelineItemFromLocalApi(thread, codexHome, item.byteOffset);
      setFullItems((current) => ({ ...current, [item.id]: fullItem }));
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : String(loadError));
    } finally {
      setLoadingFullItems((current) => {
        const next = new Set(current);
        next.delete(item.id);
        return next;
      });
  ...
  async function copyLoaded() {
    const text = items.map((item) => {
      const content = fullItems[item.id]?.text ?? item.text;
      const timestamp = item.timestamp ? ` | ${item.timestamp}` : "";
      const incomplete = (fullItems[item.id] || item).textTruncated
        ? `\n\n[${t("内容过长，当前副本仅包含已读取部分。展开该条记录可继续读取。")}]`
        : "";
      return `## ${item.label}${timestamp}\n\n${content || (item.encrypted ? t("该条推理只存储了加密内容，无法读取正文。") : "")}${incomplete}`;
    }).join("\n\n---\n\n");
    if (!text) return;
    try {
      ...
      setCopied(false);
      setError(copyError instanceof Error ? copyError.message : t("复制失败"));
    }
  }

  const filterOptions: Array<{ value: TimelineFilter; label: string }> = [
    { value: "conversation", label: t("主要内容") },
    { value: "user", label: t("用户输入") },
    { value: "commentary", label: t("思考过程") },
    { value: "assistant", label: t("最终回复") },
    { value: "reasoning", label: t("推理记录") },
  ...
  return (
    <div className="thread-timeline-panel">
      <div className="timeline-toolbar">
        <div className="timeline-filter-tabs" role="group" aria-label={t("时间线类型筛选")}>
...
function PromptVirtualList({
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: () => 230,
    overscan: 5,
    getItemKey: (index) => `${prompts[index]?.index}-${prompts[index]?.lineNumber}`
  });
  React.useEffect(() => {
    if (activeSearchIndex < 0 || activeSearchIndex >= prompts.length) return;
    virtualizer.scrollToIndex(activeSearchIndex, { align: "center" });
...
function ThreadPromptModal({
  ...
  React.useEffect(() => {
    if (contentMode !== "prompts") return undefined;
    const focusPromptSearch = (event: KeyboardEvent) => {
      if (!(event.ctrlKey || event.metaKey) || event.key.toLocaleLowerCase() !== "f") return;
      event.preventDefault();
      promptSearchInputRef.current?.focus();
      promptSearchInputRef.current?.select();
    };
    window.addEventListener("keydown", focusPromptSearch, true);
    return () => window.removeEventListener("keydown", focusPromptSearch, true);
  }, [contentMode]);
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={closeOnBackdrop}>
      <section ref={dialogRef} className="prompt-modal" role="dialog" aria-modal="true" aria-label={t("线程内容")} tabIndex={-1}>
        <header className="modal-header">
          <div>
            <span className="eyebrow">{t("线程内容")}</span>
            <h2>{thread.title}</h2>
            <p>{thread.id}</p>
          </div>
          <button className="icon-button" onClick={onClose} title={t("关闭详情窗口")} aria-label={t("关闭详情窗口")} data-dialog-initial-focus type="button">
        ...
        <div className="thread-content-tabs" role="tablist" aria-label={t("线程内容视图")} onKeyDown={(event) => {
          if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
            event.preventDefault();
            selectAdjacentContentTab(event.key === "ArrowRight" ? 1 : -1);
          }
        }}>
          <button id="thread-content-tab-timeline" aria-controls="thread-content-panel-timeline" tabIndex={contentMode === "timeline" ? 0 : -1} className={contentMode === "timeline" ? "active" : ""} onClick={() => setContentMode("timeline")} role="tab" aria-selected={contentMode === "timeline"} type="button">{t("完整时间线")}</button>
          <button id="thread-content-tab-prompts" aria-controls="thread-content-panel-prompts" tabIndex={contentMode === "prompts" ? 0 : -1} className={contentMode === "prompts" ? "active" : ""} onClick={showPrompts} role="tab" aria-selected={contentMode === "prompts"} type="button">{t("我的输入")}</button>
        </div>
        ...
        <div className="prompt-content-search" role="search" aria-label={t("搜索当前筛选的全部内容")}>
          <Search size={16} aria-hidden="true" />
          <input
            ref={promptSearchInputRef}
            value={promptSearchText}
            onChange={(event) => setPromptSearchText(event.target.value)}
            onKeyDown={(event) => {
              if (event.key === "Enter") {
                event.preventDefault();
...
function useConnectorReleaseMetadata(): ConnectorReleaseMetadata | null | undefined {
  const [metadata, setMetadata] = React.useState<ConnectorReleaseMetadata | null | undefined>(undefined);
...
function LocalApiConnectionGate({
  ...
  const connectorReleaseMetadata = useConnectorReleaseMetadata();
  const isChecking = status === "checking" || status === "launching";
  ...
  const verifierDownloadLabel = language === "en" ? "Verify download" : "校验下载文件";
  const releaseTrustMessage = connectorReleaseTrustMessage(connectorReleaseMetadata, language);
    ...
    <section className="connection-gate" aria-live="polite">
      <div className="connection-card compact">
        ...
        <div className="connector-actions">
          {hostedConsole ? (
            <a className="primary-action connector-launch" href={localConnectorLaunchUrl} onClick={onLaunch}>
              <ServerCog size={16} />
              {launchLabel}
            </a>
          ) : null}
        ...
        <details className="connection-details">
          ...
          <div className="connector-download-links">
            ...
            <a href={localConnectorVerifierDownloadUrl}><ShieldCheck size={15} />{verifierDownloadLabel}</a>
          </div>
          <p className="connector-trust-line">
            <ShieldCheck size={15} />
            <span>{releaseTrustMessage}</span>
          </p>
...
function App() {
  ...
  const hostedConnectorLaunchAttemptedRef = React.useRef(false);
  ...
  const [selectedThreadRecord, setSelectedThreadRecord] = React.useState<ThreadRecord | null>(null);
  ...
  const completeHostedConnectorLaunch = React.useCallback(async () => {
    if (await waitForLocalConnector()) {
      openLocalConsole();
      return true;
    }
    setLocalApiConnection({
      status: "blocked",
      message: language === "en"
        ? "The connector did not become reachable. If it is installed, allow the browser to open Codex Home Manager and allow Local Network Access, then retry."
        : "连接器尚未连通。若已安装，请允许浏览器打开 Codex Home Manager，并允许“本地网络访问”，然后重试。"
    });
    return false;
  }, [language]);

  const handleConnectorLaunch = React.useCallback(() => {
    rememberInstalledConnector();
    hostedConnectorLaunchAttemptedRef.current = true;
    setLocalApiConnection({ status: "launching", message: "" });
    void completeHostedConnectorLaunch();
  }, [completeHostedConnectorLaunch]);
  ...
  const checkLocalApiAccess = React.useCallback(async () => {
    if (isHostedConsole()) {
      ...
      if (await probeLocalConnector()) {
        openLocalConsole();
        return false;
      }
      if ((connectorIsKnownInstalled() || returningProductUserAtPageLoad) && !hostedConnectorLaunchAttemptedRef.current) {
        hostedConnectorLaunchAttemptedRef.current = true;
        setLocalApiConnection({ status: "launching", message: "" });
        try {
          requestInstalledConnectorLaunch();
        } catch {
          // The manual launch control remains available when the browser blocks external protocols.
        }
        await completeHostedConnectorLaunch();
        return false;
      }
      setLocalApiConnection({
        status: "blocked",
        message: language === "en"
          ? "Start the connector once. Future visits will remember it and try to start it automatically."
          : "首次请启动一次连接器；以后打开本页面时会记住并自动尝试启动。"
      });
      return false;
    }
    setLocalApiConnection({ status: "checking", message: "" });
  ...
  }, [codexHome, completeHostedConnectorLaunch, language, refreshApiToken]);

  const retryLocalApiAccess = React.useCallback(() => {
    if (isHostedConsole()) {
      rememberInstalledConnector();
      hostedConnectorLaunchAttemptedRef.current = false;
  ...
  const selectedThread = React.useMemo(() => {
    ...
    if (selectedThreadRecord?.id === selectedThreadId) return selectedThreadRecord;
    return activeSnapshot?.threads.find((thread) => thread.id === selectedThreadId) || null;
  }, [activeSnapshot, selectedThreadId, selectedThreadRecord]);
      ...
      <section className="app-main">
        ...
        {!isLocalApiConnected && !isBrowserMode && activeSection !== "api" ? (
          <LocalApiConnectionGate
            ...
            language={language}
            onLaunch={handleConnectorLaunch}
            onRetry={retryLocalApiAccess}
