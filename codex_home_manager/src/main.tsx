...
function PromptVirtualList({ prompts, filterMode }: { prompts: PromptRecord[]; filterMode: PromptFilterMode }) {
  const { t } = useI18n();
  const parentRef = React.useRef<HTMLDivElement>(null);
  const virtualizer = useVirtualizer({
  ...
  return (
    <div ref={parentRef} className="prompt-list">
        ...
        {virtualizer.getVirtualItems().map((virtualRow) => {
          ...
          return (
            <div key={`${prompt.index}-${prompt.lineNumber}`} ref={virtualizer.measureElement} data-index={virtualRow.index} className="prompt-entry-wrap" style={{ transform: `translateY(${virtualRow.start}px)` }}>
              <article className={`prompt-entry prompt-entry-${prompt.sourceType || "unknown"}`}>
        ...
        <div className="prompt-modal-content">
          ...
          {!isLoading && prompts ? (
            filteredPrompts.length
              ? <PromptVirtualList prompts={filteredPrompts} filterMode={filterMode} />
